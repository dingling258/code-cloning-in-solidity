...
contract Ownable is Context {
    ...
    constructor(address _tokenOwner) {
        require(
            ...
            "Ownable: owner is the zero address"
        );
        _owner = _tokenOwner;
...
contract Artyfact is Context, IBEP20, Ownable {
    uint256 public MAX_TOTAL_SUPPLY = 7000000 * 10 ** uint256(decimals());

    uint256 public operatorMintLimit;
    uint256 public operatorMintedAmount;
    ...
    function setMintOperator(address operator) external onlyOwner {
        mintOperator = operator;
    }

    function setOperatorMintLimits(uint256 newLimit) external onlyOwner {
        operatorMintLimit = newLimit;
        operatorMintedAmount = 0;
    }

    function mint(address to, uint256 amount) external {
        require(
            msg.sender == owner() || msg.sender == mintOperator,
            "Insufficient rights"
        );
        require(
            _totalSupply + amount <= MAX_TOTAL_SUPPLY,
            "Max total supply reached"
        );
        if (msg.sender == mintOperator) {
            operatorMintedAmount += amount;
            require(operatorMintedAmount <= operatorMintLimit, "Operator mint limit reached");
        }
        _balances[to] += amount;
        _totalSupply += amount;
    }
