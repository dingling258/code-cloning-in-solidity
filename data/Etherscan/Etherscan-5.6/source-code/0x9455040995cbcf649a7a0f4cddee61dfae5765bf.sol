...
/*
M""""""""M M""MMMMM""MM MM"""""""`MM MM""""""""`M MM""""""""`M 
Mmmm  mmmM M  MMMMM  MM MM  mmmm,  M MM  mmmmmmmM MM  mmmmmmmM 
MMMM  MMMM M         `M M'        .M M`      MMMM M`      MMMM 
MMMM  MMMM M  MMMMM  MM MM  MMMb. "M MM  MMMMMMMM MM  MMMMMMMM 
MMMM  MMMM M  MMMMM  MM MM  MMMMM  M MM  MMMMMMMM MM  MMMMMMMM 
MMMM  MMMM M  MMMMM  MM MM  MMMMM  M MM        .M MM        .M 
MMMMMMMMMM MMMMMMMMMMMM MMMMMMMMMMMM MMMMMMMMMMMM MMMMMMMMMMMM 
...
library Address{
 ...
 contract THREE is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyLimit = 500_000 * 10**9;
    uint256 public maxSellLimit = 500_000 * 10**9;
    uint256 public maxWalletLimit = 500_000 * 10**9;
    
    uint256 public genesis_block;
    
    address public treasuryWallet = 0xE2caBD4540ffedbb98A35a3c57c26B7693EfD224;
    address public stakingWallet = 0xd0AE9c188915fEeaC59e9743361A78BCF8E79fDe;

    string private constant _name = "Three Protocol";
    string private constant _symbol = "THREE";
    ...
    Taxes public taxes = Taxes(0, 2, 3, 0);
    ...
    function setTaxes(uint256 _rfi, uint256 _treasury, uint256 _liquidity, uint256 _staking) public onlyOwner {
       taxes = Taxes(_rfi,_treasury,_liquidity,_staking);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _treasury, uint256 _liquidity, uint256 _staking) public onlyOwner {
       sellTaxes = Taxes(_rfi,_treasury,_liquidity,_staking);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rTreasury, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
        (to_return.rStaking) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tTreasury-s.tLiquidity-s.tStaking;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rTreasury, uint256 rLiquidity){
        ...
        uint256 rStaking = s.tStaking*currentRate;
        rTransferAmount =  rAmount-rRfi-rTreasury-rLiquidity-rStaking;
        return (rAmount, rTransferAmount, rRfi,rTreasury,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rStaking) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tTreasury + s.tStaking);
        }
        if(s.rTreasury > 0 || s.tTreasury > 0) _takeTreasury(s.rTreasury, s.tTreasury);
        if(s.rStaking > 0 || s.tStaking > 0) _takeStaking(s.rStaking, s.tStaking);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.treasury + temp.staking) * 2;
        ...
        uint256 stakingAmt = unitBalance * 2 * temp.staking;
        if(stakingAmt > 0){
