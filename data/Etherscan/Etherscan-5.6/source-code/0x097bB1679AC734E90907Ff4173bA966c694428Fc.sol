...
contract NftStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    uint public MAX_DEPOSIT = 4;
    //max pool cap
    uint public MAX_POOL = 200;
...
/* STAKING MECHANICS */
    ...
    function setPoolCap(uint _maxPool) public onlyOwner() {
      MAX_POOL = _maxPool;
    }
    ...
    function deposit(uint256[] calldata tokenIds) external whenNotPaused noContractsAllowed nonReentrant() {
        ...
        require(tokenIds.length + IERC721(stakingDestinationAddress).balanceOf(address(this)) <= MAX_POOL, "Max pool reached!");
        require(_deposits[msg.sender].length() + tokenIds.length <= MAX_DEPOSIT, "Max deposit reached!");
