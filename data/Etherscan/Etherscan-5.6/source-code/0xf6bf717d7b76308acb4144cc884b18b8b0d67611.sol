...
https://t.me/FOMOonETH
https://x.com/FOMOonETH
https://FOMOonETH.com/

  o__ __o__/_   o__ __o        o          o        o__ __o      
 <|    v       /v     v\      <|\        /|>      /v     v\     
 < >          />       <\     / \\o    o// \     />       <\    
  |         o/           \o   \o/ v\  /v \o/   o/           \o  
  o__/_    <|             |>   |   <\/>   |   <|             |> 
...
contract FOMO is Ownable, ERC20 {
    ...
    event set_Receivers(address marketingFeeReceiver, address buybackFeeReceiver,address burnFeeReceiver,address fomofeeReceiver);
    ...
    uint256 public _maxTxAmount = _totalSupply.mul(13).div(1000);
    uint256 public _maxWalletToken = _totalSupply.mul(13).div(1000);
    ...
    uint256 private fomofee         = 1;
    uint256 private buybackFee      = 0; 
    uint256 private burnFee         = 0;
    uint256 public totalFee         = buybackFee + marketingFee + liquidityFee + fomofee + burnFee;
    uint256 private feeDenominator  = 100;

    uint256 sellpercent = 1000;
    uint256 buypercent = 600;
    uint256 transferpercent = 100; 

    address private autoLiquidityReceiver;
    address private marketingFeeReceiver;
    address private fomofeeReceiver;
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0x6B7C500eA895deCee00cC81bB48601B827521Be1;
        fomofeeReceiver = msg.sender;
   ...
   function clearStuckCoins(address tokenAddress, uint256 tokens) external returns (bool success) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHdev = amountETH.mul(fomofee).div(totalETHFee);

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountETHMarketing}("");
        (tmpSuccess,) = payable(fomofeeReceiver).call{value: amountETHdev}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _marketingFee, uint256 _fomofee, uint256 _burnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        marketingFee = _marketingFee;
        fomofee = _fomofee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_marketingFee).add(_fomofee).add(_burnFee);
    ...
    function setMarketingReceiver(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _fomofeeReceiver, address _burnFeeReceiver, address _buybackFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        fomofeeReceiver = _fomofeeReceiver;
        burnFeeReceiver = _burnFeeReceiver;
        buybackFeeReceiver = _buybackFeeReceiver;

        emit set_Receivers(marketingFeeReceiver, buybackFeeReceiver, burnFeeReceiver, fomofeeReceiver);
    }
