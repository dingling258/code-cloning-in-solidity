...
contract SpleenSquad is ERC721A, Ownable, ERC2981 {
    error SPLEENSQUAD_MINT_PAUSED();
    error SPLEENSQUAD_ZERO_ADDRESS();
    error SPLEENSQUAD_ZERO_PRICE();
    error SPLEENSQUAD_BLANK_URI();
    error SPLEENSQUAD_NOT_VERIFIED();
    error SPLEENSQUAD_INVALID_MERKLE_ROOT();
    error SPLEENSQUAD_MERKLE_ROOT_NOT_SET();
    error SPLEENSQUAD_NO_FUNDS_TO_WITHDRAW();
    error SPLEENSQUAD_WITHDRAWAL_FAILED();
    error SPLEENSQUAD_NONEXISTENT_TOKEN();
    error SPLEENSQUAD_INSUFFICIENT_FUNDS();
    error SPLEENSQUAD_NOT_A_TOKEN_OWNER();
    error SPLEENSQUAD_WALLET_LIMIT_REACHED();
    error SPLEENSQUAD_MINT_TRANSACTION_LIMIT_REACHED();
    ...
    // PRICES
    uint256 private mint_price = 0.1 ether;
    uint256 private wl_mint_price = 0.08 ether;
    uint256 private early_bird_mint_price = 0.05 ether;

    // LIMIT
    uint8 private tx_limit = 10;
    uint8 private wallet_limit = 5;
    uint8 private wl_wallet_limit = 3;
    uint8 private vip_wallet_limit = 3;
    ...
    constructor(address _royaltyAddress, uint96 _royaltyBips) ERC721A("Spleen Squad", "SPLSQ") Ownable(msg.sender) {
    ...
    modifier checkZeroAddress(address _address) {
        if (_address == address(0)) {
            revert SPLEENSQUAD_ZERO_ADDRESS();
        }
        _;
    }
    ...
    modifier isNotPaused() {
        if (saleConfig == SaleConfig.PAUSED) {
            revert SPLEENSQUAD_MINT_PAUSED();
        }
        _;
    }
    ...
    modifier onlyTokenOwner(uint256 tokenId) {
        if (ownerOf(tokenId) != msg.sender) {
            revert SPLEENSQUAD_NOT_A_TOKEN_OWNER();
        }
        _;
    }
    ...
    modifier isInMintTransactionLimit(uint256 _amount) {
        if (_amount > tx_limit) {
            revert SPLEENSQUAD_MINT_TRANSACTION_LIMIT_REACHED();
        }
        _;
    }

    modifier hasMerkleRoot() {
        if (merkleRoot.length == 0) {
            revert SPLEENSQUAD_MERKLE_ROOT_NOT_SET();
        }
        _;
    }
    ...
    function EarlyBirdMintPrice() external view returns (uint256) {
        return early_bird_mint_price;
    }
    ...
    function isEarlyBird() external view returns (bool) {
        return saleConfig == SaleConfig.EARLY;
    }
    ...
    function getMaxLimit() external view returns (uint8) {
        if (saleConfig == SaleConfig.EARLY) {
            return wallet_limit;
        }

        if (saleConfig == SaleConfig.PRESALE) {
            return wl_wallet_limit;
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (!_exists(tokenId)) {
            revert SPLEENSQUAD_NONEXISTENT_TOKEN();
        }
    ...
    function setSaleConfigToEarlyBird() external onlyOwner {
        saleConfig = SaleConfig.EARLY;
    }

    function setSaleConfigToPresale() external onlyOwner {
        saleConfig = SaleConfig.PRESALE;
    }

    function setSaleConfigToPublic() external onlyOwner {
    ...
    function setMetaDataURI(string calldata _baseURI) external onlyOwner isNotBlankURI(_baseURI) {
        baseURI = _baseURI;
    }
    ...
    function setWLMintPrice(uint256 _price) external onlyOwner checkZeroPrice(_price) {
        wl_mint_price = _price;
    }

    function setEarlyBirdMintPrice(uint256 _price) external onlyOwner checkZeroPrice(_price) {
        early_bird_mint_price = _price;
    }

    function setMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        if (_merkleRoot.length == 0) {
            revert SPLEENSQUAD_INVALID_MERKLE_ROOT();
        }
    ...
    function mintForSelf(uint256 _amount) external payable onlyOwner isInMintTransactionLimit(_amount) {
        _mint(msg.sender, _amount);
    }

    function mintForAddress(address _receiver, uint256 _amount)
        external
        payable
        onlyOwner
    ...
    function mint(uint256 _amount) external payable isNotPaused isInMintTransactionLimit(_amount) {
        if (_numberMinted(msg.sender) + _amount > wallet_limit) {
            revert SPLEENSQUAD_WALLET_LIMIT_REACHED();
        }
        if (msg.value < mint_price * _amount) {
            revert SPLEENSQUAD_INSUFFICIENT_FUNDS();
        }
        _mint(msg.sender, _amount);
    }

    function earlyBirdmint(uint256 _amount) external payable isNotPaused isInMintTransactionLimit(_amount) {
        if (_numberMinted(msg.sender) + _amount > wallet_limit) {
            revert SPLEENSQUAD_WALLET_LIMIT_REACHED();
        }
        if (msg.value < early_bird_mint_price * _amount) {
            revert SPLEENSQUAD_INSUFFICIENT_FUNDS();
        }
        _mint(msg.sender, _amount);
    }
        ...
        isInMintTransactionLimit(_amount)
    {
        if (!verify(_proof, msg.sender)) {
            revert SPLEENSQUAD_NOT_VERIFIED();
        }
        if (_numberMinted(msg.sender) + _amount > wl_wallet_limit) {
            revert SPLEENSQUAD_WALLET_LIMIT_REACHED();
        }
    ...
    function withdrawFunds() external onlyOwner {
        if (address(this).balance == 0) {
            revert SPLEENSQUAD_NO_FUNDS_TO_WITHDRAW();
        }
        (bool success,) = owner().call{value: address(this).balance}("");
        if (!success) {
            revert SPLEENSQUAD_WITHDRAWAL_FAILED();
        }
    }
}