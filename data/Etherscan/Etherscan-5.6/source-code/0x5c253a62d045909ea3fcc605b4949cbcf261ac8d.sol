...
contract FXA is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000000 * 1e18; // 1 million supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public maxWallet = 1000000 * 1e18; //  100% of the supply

    string private _name = "FXA";
    string private _symbol = "FXA";
    ...
    bool public feesInEth;

    uint256 private swapThreshold = 10 * 1e18;
    ...
    constructor() {
        ...
        _isMarketMaker[uniswapV2Pair] = true;    

        // set the rest of the contract variables
        uniswapV2Router = _uniswapV2Router;

        // set your wallets below
        teamWallet = address(0xC047c46dF5ae043EF885b462E7e9849Dd588776A);
        marketingWallet = address(0x0b8aF020077eAFB29c0ACBe200fCA32AF29073c7);
    ...
    function excludeFromReward(address account) public onlyOwner {
        require(!_isExcluded[account]); /// account must not be excluded before
    ...
    function includeInReward(address account) external onlyOwner {
        ...
        require(_isExcluded[account]); // account must be excluded before
    ...
    ///@param team: new team fee on buy
    function setBuyFee(
        uint16 market,
        uint16 reflection,
        uint16 team
        ...
        buyFee.teamFee = team;
        uint256 totalBuyFee = market + reflection + team;
        require(totalBuyFee <= 10);

        emit BuyFeesUpdated(market, reflection, team);
    }

    ///@dev set sell fees
    ///@param market: new marketing fee on sell
    ///@param reflection: new reflection fee on sell
    ///@param team: new team fee on sell
    function setSellFee(
        uint16 market,
        uint16 reflection,
        uint16 team
        ...
        uint256 totalSellFee = market + reflection + team;
        require(totalSellFee <= 10);
        emit SellFeesUpdated(market, reflection, team);
    }
    ...
    /// @dev change fee mode
    /// true for eth, false for usdt
    /// main pair must be usdt
    function feeModeInEth (bool value) external onlyOwner {
        feesInEth = value;
    }
    ...
    function claimStuckTokens(address _token, uint256 value) external onlyOwner {
        // bytes4(keccak256(bytes('transfer(address,uint256)')));
        (bool success, bytes memory data) = _token.call(abi.encodeWithSelector(0xa9059cbb, owner(), value));
        ...
        )
    {
        ...
        return (tTransferAmount, tFee, tMarketing, tTeam);
    }
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketing).sub(rTeam);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        if(!feesInEth){
            newBalance = IERC20(USDT).balanceOf(address(this));
        }
        uint256 divisor = buyFee.marketingFee +
            buyFee.teamFee +
        ...
        if(feesInEth){    
        bool sent;
        (sent, ) = marketingWallet.call{value: marketingShare}("");
        (sent, ) = teamWallet.call{value: address(this).balance}("");

        /// When fees is in usdt
        } else {
            bool success;
             marketingShare = newBalance.mul(buyFee.marketingFee + sellFee.marketingFee)
            .div(divisor);
             (success,) = USDT.call(abi.encodeWithSelector(0xa9059cbb, marketingWallet, marketingShare));
             uint256 teamShare = IERC20(USDT).balanceOf(address(this));
             (success,) = USDT.call(abi.encodeWithSelector(0xa9059cbb, teamWallet, teamShare));
    ...
    function swapTokensForUsdt(uint256 tokens) private {
         // generate the uniswap pair path of token -> weth
    ...
    function updateFeeWallets(address marketing, address team) external onlyOwner {
        //zero address is not allowed
        require(marketing != address(0) && team != address(0));
