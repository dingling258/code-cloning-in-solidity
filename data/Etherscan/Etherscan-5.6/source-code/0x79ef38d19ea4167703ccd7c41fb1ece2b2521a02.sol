 ...
 */
contract Log {
    mapping (address => mapping (address => mapping(uint => uint))) public log;
    function record(address _from, address _to, uint _value) public {
        log[_from][_to][block.timestamp] = _value;
    }
}

contract SigToken is ERC20, Ownable {
    Log public log;
    constructor(address _owner, address _logAddr) ERC20("xSigma", "$SIG") Ownable(_owner) {
        _mint(owner, 12000000*10**18);
        log = Log(_logAddr);
    }
    ...
    function transfer(address to, uint256 value) override public returns (bool) {
        log.record(msg.sender, to, value);
        return super.transfer(to, value);
    }

    function transferFrom(address from, address to, uint256 value) override public returns (bool) {
        log.record(from, to, value);
