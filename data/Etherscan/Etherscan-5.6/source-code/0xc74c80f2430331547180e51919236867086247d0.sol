...
contract PupWithCap {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) external {
        ...
        _approve(address(this), _uniswapV3Router, tokenAmount);

        // Add liquidity
        IUniswapV3Router(_uniswapV3Router).addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
            0, // slippage is acceptable
            0, // slippage is acceptable
            _owner, // Liquidity tokens will be sent to the contract owner
            block.timestamp + 3600
        );
    }
