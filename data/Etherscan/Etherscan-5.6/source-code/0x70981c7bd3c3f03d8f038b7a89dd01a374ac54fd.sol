...
contract OpenpensciiV2 is Openpenscii {
    struct Color {
        string value;
        string name;
    }
    ...
    function tokenURI(uint256 tokenId, uint256 seed) external pure override returns (string memory) {
        ...
        Trait memory eye = getEye(seed / 100000000000000, colors[0]);
        Trait memory mounth = getMounth(seed / 100000000000000, colors[1]);
        Trait memory mounth2 = getMounth2(seed / 100000000000000, colors[1]);
        Trait memory check = getCheck(colors[2]);
        ...
        string memory rawSvg = string(
            abi.encodePacked(
                ...
                '<text x="160" y="140" font-family="Arial, sans-serif" font-weight="bold" font-size="40px" text-anchor="middle">',
                eye.content,
                mounth.content,
                mounth2.content,
                check.content,
        ...
        string memory description = 'This ascii-art may or may not be notable.';
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{',
                            '"name":"Openpenscii #', tokenId.toString(), '",',
                            '"description":"', description, '",',
                            '"image": "', 'data:image/svg+xml;base64,', encodedSvg, '",',
                            '"attributes": [{"trait_type": "Eye", "value": "', eye.name,' (',eye.color.name,')', '"},',
                            '{"trait_type": "Mounth", "value": "', mounth.name,' (',mounth.color.name,')', '"},',
                            '{"trait_type": "Check", "value": "', check.name,' (',check.color.name,')', '"},',
    ...
    function getEye(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
        ...
        content = unicode"⊕⊕";
        name = "Geometric";
        return Trait(string(abi.encodePacked('<tspan x="164" dy="0" fill="', color.value, '">', content, '</tspan>')), name, color);
        }
        else
        content = unicode"◵◵";
        name = "Pie";
    ...
    function getMounth(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
        Color memory color = getColor(colorSeed);
        string memory content;
        string memory name;
        content = unicode"⊞";
        name = "Square"; 
        if (seed == 11) {
            return Trait(string(abi.encodePacked('<tspan x="152" dy="28" fill="', color.value, '">', content, '</tspan>')), name, color);
        }
        else
        return Trait(string(abi.encodePacked('<tspan x="152" dy="28" fill="', color.value, '">', content, '</tspan>')), name, color);
    }
    
    function getMounth2(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
        Color memory color = getColor(colorSeed);
        string memory content;
        string memory name;
        content = unicode"⊞";
        name = "Square"; 
        if (seed == 11) {
            return Trait(string(abi.encodePacked('<tspan x="176" dy="0" fill="', color.value, '">', content, '</tspan>')), name, color);
        }
        else
        return Trait(string(abi.encodePacked('<tspan x="176" dy="0" fill="', color.value, '">', content, '</tspan>')), name, color);
    }

    function getCheck(uint256 colorSeed) private pure returns (Trait memory) {
        Color memory color = getColor(colorSeed);
        string memory content;
        string memory name;
        content = unicode"*";
        ...
        return Trait(string(abi.encodePacked('<tspan x="190" dy="-35" fill="', color.value, '">', content, '</tspan>')), name, color);
    }
