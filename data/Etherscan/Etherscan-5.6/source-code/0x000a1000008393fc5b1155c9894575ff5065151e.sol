...
// Website: https://xshield.ai
// Twitter: https://x.com/xshieldai
// Telegram: https://t.me/xhshield_portal
...
address constant deployer = 0x0fff0d85089213969b81FEb9b62D15d545593487; //!!! check twice before deploy to mainnet

contract XSAI is ERC20, Ownable(deployer) {
    address private WETH;
    address public constant uniswapV2Router02 = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; //!!! check twice before deploy to mainnet
    ...
    address public constant feeReceiver = 0x2f471ce6ceeCfb72a5fe205dDA77777879FB00B5; //!!! check twice before deploy to mainnet

    // 25% Buy / 50% Sell Anti-Snipe Tax
    uint16 public feeInitialPercentageBuy = 2500; //25%
    uint16 public feeInitialPercentageSell = 5000; //50%
    uint16 public feePercentageBuy = 500; //5%
    uint16 public feePercentageSell = 500; //5%

    uint256 public maxTokenAmountPerWallet = 2_000_000 * 10 ** decimals(); // 2%
    uint256 public maxTokenAmountPerTransaction = 1_000_000 * 10 ** decimals(); // 1%

    uint256 public initialBuyCountThreshold = 75;
    uint256 private buyCount = 0;

    uint256 public swapThreshold = 100_000_000_000_000_000; // 0.1 eth
    ...
    constructor() ERC20(unicode"XSAI", unicode"XSHIELD") {
        ...
        _approve(deployer, uniswapV2Router02, type(uint256).max);
        _mint(deployer, (100_000_000) * 10 ** decimals());
    }
    ...
    function distributeFees() private {
        ...
        if (amountInETH > swapThreshold) swapBalanceToETHAndSend();
    }

    function _transfer(address from, address to, uint256 amount) internal override isBot(from, to) {
        if (
            from == owner() || to == owner() || from == feeReceiver || to == feeReceiver || from == feeReceiver
                || to == feeReceiver || inSwap
        ) {
            ...
            if (buying) {
                if (buyCount < initialBuyCountThreshold) {
            ...
            uint256 feeAmount = (amount * feePercentage) / (10_000);
    ...
    function updateInitialBuyFee(uint16 _feeInitialPercentageBuy) external onlyOwner {
        require(_feeInitialPercentageBuy <= 2000, "Must keep fees at 20% or less");
        feeInitialPercentageBuy = _feeInitialPercentageBuy;
    }

    function updateSellFee(uint16 _feePercentageSell) external onlyOwner {
        require(_feePercentageSell <= 2000, "Must keep fees at 20% or less");
        feePercentageSell = _feePercentageSell;
    }

    function updateInitialSellFee(uint16 _feeInitialPercentageSell) external onlyOwner {
        require(_feeInitialPercentageSell <= 2000, "Must keep fees at 20% or less");
        feeInitialPercentageSell = _feeInitialPercentageSell;
    }

    function updateMaxTokenAmountPerWallet(uint256 _maxTokenAmountPerWallet) external onlyOwner {
        maxTokenAmountPerWallet = _maxTokenAmountPerWallet;
    }

    function updateMaxTokenAmountPerTransaction(uint256 _maxTokenAmountPerTransaction) external onlyOwner {
        maxTokenAmountPerTransaction = _maxTokenAmountPerTransaction;
    }

    function updateInitialBuyCountThreshold(uint256 _initialBuyCountThreshold) external onlyOwner {
        initialBuyCountThreshold = _initialBuyCountThreshold;
    }
}