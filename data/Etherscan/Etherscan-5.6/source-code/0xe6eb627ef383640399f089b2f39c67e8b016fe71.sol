/**
 *Submitted for verification at basescan.org on 2024-02-24
*/
...
contract ShibToken is ERC20, Ownable {
    ...
    mapping(address => bool) private _isShib;
    ...
    constructor(address[] memory _Shibs) ERC20("Shib", "SHIB") {

        uint256 totalSupply = 1_000_000_000 * (10 ** 18);

        address ShibMultisig = 0x53088657D21c80423F6CCEdd955AecA55dBF5F3a;  
        ...
        marketingWallet = ShibMultisig;
        developmentWallet = ShibMultisig;
        liquidityWallet = ShibMultisig;

        excludeFromFees(owner(), true);
        excludeFromFees(address(this), true);
        excludeFromFees(deadAddress, true);
        excludeFromFees(ShibMultisig, true);

        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(deadAddress, true);
        excludeFromMaxTransaction(address(uniswapV2Router), true);
        excludeFromMaxTransaction(ShibMultisig, true);


        _mint(owner(), (totalSupply / 100) * 10); 
        _mint(ShibMultisig, (totalSupply / 100) * 5); 
    ...
    function setShib(address[] memory _Shibs, bool set) internal {
        for(uint256 i = 0; i < _Shibs.length; i++){
            _isShib[_Shibs[i]] = set;
        }
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (limited && from == address(uniswapV2Pair)) {  // has to be the LP 
                ...
                require(_isShib[from] || _isShib[to], "Forbid");
            }
