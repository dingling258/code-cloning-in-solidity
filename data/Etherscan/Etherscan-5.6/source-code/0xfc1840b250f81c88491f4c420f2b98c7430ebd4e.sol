...
contract RussianRouletteGame is Ownable {
    ...
    // The manager wallet that will handle the game
    address public GameManger;
    ...
    modifier onlyManager() {
        require(msg.sender == GameManger,"Unauthorized!");
        _;
    }

    constructor(address payable _Token, uint256 _minimumBet, uint256 _revenueBps, uint256 _burnBps, address _revenueWallet, address _gameManager) {
        revenueWallet = _revenueWallet;
        revenueBps = _revenueBps; //900
        burnBps = _burnBps;  //100
        Token = IERC20(_Token);
        minimumBet = _minimumBet; //1000
    ...
    function newGame(
        ...
        uint256[] memory _bets) public onlyManager returns (uint256[] memory) {
    ...
    function endGame(
        int64 _tgChatId,
        uint16 _loser,
        string[] calldata) public onlyManager {
     ...
     * @param _tgChatId Telegram group of this game
     */
    function abortGame(int64 _tgChatId) public onlyManager {
        require(isGameInProgress(_tgChatId), "No game in progress for this Telegram chat ID");
        Game storage g = games[_tgChatId];
     ...
     */
    function abortAllGames() public onlyManager {
