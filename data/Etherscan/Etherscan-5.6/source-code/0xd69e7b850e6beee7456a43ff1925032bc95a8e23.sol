...
contract Database is ERC20, Ownable {
    ...
    mapping(address => bool) public excludedMaxPerTxn;
    ...
    constructor() ERC20("Database", "DATA") {
        devAddress = owner();
        ...
        excludedMaxPerTxn[owner()] = true;
        excludedMaxPerTxn[address(this)] = true;
        ...
        excludedMaxPerTxn[address(0xdead)] = true;
        excludedMaxPerTxn[address(swapRouter)] = true;
        excludedMaxPerTxn[address(swapPair)] = true;

        excludedTax[address(0xdead)] = true;

        buyTaxMarketing = 20;
        buyTaxDev = 20;
        buyTaxTotal = buyTaxMarketing + buyTaxDev;

        sellTaxMarketing = 30;
        sellTaxDev = 30;
        sellTaxTotal = sellTaxMarketing + sellTaxDev;
    ...
    function updateBuyTax(uint256 newMarketFee, uint256 newDevFee)
        external onlyOwner {
        buyTaxMarketing = newMarketFee;
        buyTaxDev = newDevFee;
        buyTaxTotal = buyTaxMarketing + buyTaxDev;
        require(buyTaxTotal <= 95, "Must keep tax at 95% or less");
    }

    function updateSellTax(uint256 newMarketFee, uint256 newDevFee)
        external onlyOwner {
        sellTaxMarketing = newMarketFee;
        sellTaxDev = newDevFee;
        sellTaxTotal = sellTaxMarketing + sellTaxDev;
        require(sellTaxTotal <= 95, "Must keep tax at 95% or less");
    }
    ...
    function _transfer(
        ...
        if (bTax) {
            if (ammPairs[to] && sellTaxTotal > 0) {
                ...
                devTokens += (fees * sellTaxDev) / sellTaxTotal;
                marketingTokens += (fees * sellTaxMarketing) / sellTaxTotal;
            }
            else if (ammPairs[from] && buyTaxTotal > 0) {
                fees = amount.mul(buyTaxTotal).div(100);
                devTokens += (fees * buyTaxDev) / buyTaxTotal;
                marketingTokens += (fees * buyTaxMarketing) / buyTaxTotal;
            }
            if (fees > 0)
