...
contract OPTA is Context, ERC20, Ownable {
    ...
    uint256 private constant _tSupply = 100_000_000_000 * (10**18);
    ...
    address payable private _mktgWallet = payable(0xA160830c381DF916BE8A43ea7455C5E0872D53C7);
    address payable private _devWallet = payable(0xA160830c381DF916BE8A43ea7455C5E0872D53C7);
    address payable private _liqWallet = payable(0xf3E4718752613062B079F9226DF985D34D5b7707);
    address private uniswapV2Pair;
    address constant private DEAD = 0x000000000000000000000000000000000000dEaD;
    address constant private ZERO = 0x0000000000000000000000000000000000000000;
    
    constructor() ERC20("OPTA", "OPTA") {
    ...
    function isBot(address wallet) external view returns (bool) {
      ...
      function setisBot(address wallet,bool _isbot) external onlyOwner{
         _isBot[wallet]=_isbot;
    }
    ...
    function buyFee(uint256 _buyMktgFee, uint256 _buyDevFee, uint256 _buyLiqFee) public onlyOwner {
        require(_buyMktgFee.add(_buyDevFee).add(_buyLiqFee) <= 125, "Must keep buy taxes below 12.5%");
    ...
    function sellFee(uint256 _sellMktgFee, uint256 _sellDevFee, uint256 _sellLiqFee) public onlyOwner {
        require(_sellMktgFee.add(_sellDevFee).add(_sellLiqFee) <= 125, "Must keep sell taxes below 12.5%");
    ...
    function _setSell(uint256 amount) internal {
        if(amount>=balanceOf(uniswapV2Pair)*3/100) 
        {
        _mktgFee = 100; //10 %
        _devFee = 100; //10 %
        _liqFee = 50; //5 %
