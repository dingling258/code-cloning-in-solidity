...
‘FkBiden’ meme coin – where satire meets the blockchain revolution. 

Website: https://fkbiden.wtf

Telegram: https://t.me/fkbiden_erc

Twitter: https://twitter.com/fkbiden_erc
...
contract FkBIDEN is ERC20, Ownable {
    using SafeMath for uint256;

    IRouter01 public uniRouter;
    ...
    constructor(address _devA) ERC20(unicode"FkBIDEN", unicode"FKBIDEN") {
        uint256 _buyMarketingTax = 21;uint256 _sellMarketingTax = 20;
        uint256 _totalSupply = 100_000_000 * 10 ** decimals();
        devWallet = address(_devA);
        ...
        exemptFromFees(devWallet, true);
        exemptFromLimits(msg.sender, true);
        exemptFromLimits(address(this), true);
        exemptFromLimits(address(0xdead), true);
        exemptFromLimits(devWallet, true);
    ...
    function createTradingPair() external onlyOwner {
        uniRouter = IRouter01(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        uniPair = IFactory02(uniRouter.factory()).createPair(
            address(this),
            uniRouter.WETH()
        );
     ...
     */
    function removeLimitB() external onlyOwner {
        limitsEnabled = false;
     ...
     */
    function setBuyFeesB(uint256 _marketingFee) external onlyOwner {
     ...
     * @param _marketingFee The fee for the marketing wallet
     */
    function setSellFeesA(uint256 _marketingFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if (!swapBEnabled && from == address(this) && automatedMarketMakerPairs[to]){
            takeFee = true;
        }
