/**
 *Submitted for verification at testnet.bscscan.com on 2024-04-01
*/
...
contract InfinidiumPresale is Ownable {
    ...
    //only whitelisted addresses can participate in presale if this is true
    ...
    function buyTokensEth() payable public {
        ...
        if(isWhitelistPresale){
            require(isWhitelisted(msg.sender), "This presale stage is only for whitelisted addresses");
        }
        ...
        // IERC20(tokenAddress).safeTransfer(msg.sender, tokenAmount * 1e18);
    ...
    function claim(uint256 _currentPhase) external {
        ...
        require(claimableReward > 0, "You don't have any claimable token" );
        claimedAmount[msg.sender][_currentPhase] = deservedAmount[msg.sender][_currentPhase];
        IERC20(tokenAddress).safeTransfer(msg.sender, claimableReward * 1e18);
    }
    ...
    function estimatedToken (uint256 _weiAmount) public view returns (uint256) {
        ...
        uint256 ethPerTokens = usdToEth(tokenPriceInUsd);

        return _weiAmount / ethPerTokens;
