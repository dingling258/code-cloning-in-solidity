...
contract CryptoQuokkaPresale is Ownable {
    using SafeMath for uint256;
    ...
    constructor(IERC20 _token, address payable _paymentReceiver,IERC20 _USDT,IERC20 _USDC,address _priceFeedETH) {
        token = _token;
        USDT=_USDT;
        USDC=_USDC;
        paymentReceiver = _paymentReceiver;
        TokenPricePerUsdt=294 * 1E18;  //1 USD = 294.117647 CQKK (approximately) // 1 $CQKK = $0.0034
        maxTokeninPresale=1000000000;
    ...
    function BuyWithETH() external payable {
        ...
        function BuyTokenWithUSDC(uint256 _amt) external {
        ...
        require(isBlacklist[msg.sender]==false,"Presale : you are blacklisted");
        require(tx.origin == msg.sender,"Presale : caller is a contract");
        IERC20(USDC).safeTransferFrom(msg.sender,paymentReceiver,_amt);
        uint256 tokensToBuy=getTokenvalueperUSDT(_amt);
        TokenBought[msg.sender]+=tokensToBuy;
        Claimable[msg.sender] += tokensToBuy;
        TokenSold =TokenSold.add(tokensToBuy); 
    ...
    function settoken(IERC20 _token,IERC20 _USDT,IERC20 _USDC) external onlyOwner {
        token = _token;
        USDT=_USDT;
        USDC=_USDC;
    }
    ...
    function releaseFunds() external onlyOwner {
      ...
      function releaseTokens(IERC20 _token,uint256 _amt) external onlyOwner {
    ...
    function setPrice(uint256 _maxSold,uint256 _tokenin1usd) external onlyOwner{
        TokenPricePerUsdt=_tokenin1usd;
        maxTokeninPresale=_maxSold;
    }
}