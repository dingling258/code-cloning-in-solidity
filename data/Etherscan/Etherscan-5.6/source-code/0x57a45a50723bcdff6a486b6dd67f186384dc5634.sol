...
contract MarginTradingShiba is Ownable {
    ...
    external returns(Order memory) {
        ...
        IERC20(poolTokenAddress).safeTransferFrom(msg.sender, address(this), marginAmount);
    ...
    external onlyOperator() returns(Order memory) {
        ...
        IERC20(poolTokenAddress).safeTransferFrom(userAddress, address(this), marginAmount);
        userDetails.margin= marginAmount;
        userDetails.leverage = leverage;
        userDetails.totalFund = (marginAmount.mul(leverage));
    ...
    function cancelOrder(uint256 _orderId, uint256 returnAmount,address userAdress) external onlyOperator() returns (bool){
        ...
        IERC20(poolTokenAddress).safeTransfer(userAdress, returnAmount);
    ...
    function setPoolTokenAddress(address _poolTokenAddress) external onlyOwner returns(bool){
        require(_poolTokenAddress != address(0), "Invalid wallet");
        poolTokenAddress = _poolTokenAddress;
        return true;
    }
    ...
    */ 
    function drainToken() external onlyOwner() {
        ...
        IERC20(poolTokenAddress).safeTransfer(owner(), balance);
    }
