...
contract Token is Context, IERC20, Ownable, SafeErc20 {
    ...
    uint256 _totalSupply = 10_000_000 * 10 ** _decimals;    // TEN Million Supply

    // Anti Whale Mechanism with 0.5% of Max Bag and Transaction
    uint256 public maxTransaction =  _totalSupply.mul(5).div(1000);     
    uint256 public maxWallet = _totalSupply.mul(5).div(1000);        
    ...
    address private marketingWallet = address(0x2B216a232B592A74c875c223C8Ed58d7611E7624);
    address private developmentWallet = address(0x2B216a232B592A74c875c223C8Ed58d7611E7624);
    ...
    function setInfo(string memory name_, string memory symbol_) external onlyOwner {
        _name = name_;
        _symbol = symbol_;
    }
    ...
    function swapBack(uint contractBalance) internal swapping {
        ...
        if(amountReceived > 0) {
            uint split = amountReceived / 2;
            uint half  =  amountReceived - split;
            payable(marketingWallet).transfer(split);
            payable(developmentWallet).transfer(half);
        }
    }
    ...
    function setFee(uint _buySide, uint _sellSide) external onlyOwner {    
        buyFee = _buySide;
        sellFee = _sellSide;
        require(_buySide <= 20 && _sellSide <= 20,"Max Tax Limit is 2%");
    }
