...
contract EvoNFT001 is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  address public revealOwner = 0x13b8C5180188dBC79b8F88fC41A14fB73867D953;
  mapping(uint256 => uint) public evolutions;
  ...
  function repayment(address to) public {
    require(msg.sender == revealOwner,"not revealOwner");
  ...
  function incrementEvolutions(uint256 tokenId) public {
    require(msg.sender == revealOwner,"not revealOwner");
    if (evolutions[tokenId] >= 0) {
        evolutions[tokenId] = evolutions[tokenId] + 1;
    }
  }

  function setRevealOwner(address _newRevealOwner) public onlyOwner {
    revealOwner = _newRevealOwner;
  }

  function getRevealOwner() public view returns (address) {
    return revealOwner;
  }

  function setEvolutions(uint256 tokenId, uint newEvolutions) public onlyOwner {
    evolutions[tokenId] = newEvolutions;
  }
