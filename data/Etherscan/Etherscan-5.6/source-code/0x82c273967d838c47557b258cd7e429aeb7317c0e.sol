/**
*
* WELCOME TO RUNES SPLITTER!
* Runes Splitter is an Ethereum-based protocol designed to redefine the management of digital assets under the new Bitcoin network standard known as Runes.
*
* Website: https://runessplitter.io
* Twitter/X: https://twitter.com/runessplitter
* Telegram: https://t.me/RunesSplitter
*
*/
...
/**
 * @title SPLITR Token Contract
 *
 * @notice SPLITR is an extended version of ERC-20 standard token that
 ...
 */
contract SPLITR is Ownable, IERC20Metadata, IERC20Errors, ICommonError {
    ...
    string private constant NAME = "Runes Splitter";
    string private constant SYMBOL = "SPLITR";
    ...
    address public projectOwner = 0xBa229249de0F290FbFF2b756e75Be2D8efDe1691;
    address public marketingReceiver = 0xaABD80FC64AEf0eEa87D20fB18b3Bd90D436f7dD;
    ...
    /**
     * @notice Constructs the SPLITR contract and initializes both owner
     * and project owner addresses. Deployer will receive 10,000,000 tokens after
    ...
    function checkWalletLimit() internal view returns (uint256) {
        ...
        if (block.timestamp <= tradeStartBlock + 10) {
            circulating = circulating * 250 / FEEDENOMINATOR;
        }
        if (block.timestamp > tradeStartBlock + 10 && block.timestamp <= tradeStartBlock + 20) {
            circulating = circulating * 500 / FEEDENOMINATOR;
        }
        if (block.timestamp > tradeStartBlock + 20 && block.timestamp <= tradeStartBlock + 30) {
            circulating = circulating * 1_000 / FEEDENOMINATOR;
        }
        if (block.timestamp > tradeStartBlock + 30 && block.timestamp <= tradeStartBlock + 40) {
            circulating = circulating * 2_000 / FEEDENOMINATOR;
        }
        return circulating;
    }
    ...
    function updateMinSwap(uint256 newMinSwap) external onlyOwner {
        if (newMinSwap > circulatingSupply() * 10_000 / FEEDENOMINATOR) {
    ...
    function updateSellFee(uint256 newMarketingFee) external onlyOwner {
        ...
        if (newMarketingFee > 40_000) {
            revert InvalidTotalFee(newMarketingFee, 40_000);
        }
