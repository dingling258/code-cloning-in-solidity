...
// File: interface/IERC20WithDecimals.sol
...
contract ICODZY is IICO, Ownable {
    ...
    constructor (address initialOwner, address _tokenAddress, uint256 _tokenPrice) Ownable(initialOwner) {
        require(address(_tokenAddress) != address(0x0), "ICO: tokenAddress is invalid");
        require(_tokenPrice > 0, "ICO: tokenPrice invalid");

        token = IERC20WithDecimals(_tokenAddress);
        emit SetToken(msg.sender, _tokenAddress);
        price = _tokenPrice;
        emit UpdatePrice(msg.sender, _tokenPrice);
    }

    function setToken(address _token) internal {
        if (address(token) != address(0x0)) {
            require(address(token) != _token, "ICO: This token is already set");
            require(token.balanceOf(address(this)) == 0, "ICO: Withdraw token before changing the address");
        }
