...
Telegram : https://t.me/orion_AI_portal

Website: https://orionai.dev/

Twitter/X: https://twitter.com/OAI_ETH
    ...
    Ownable
{
	...
	string private constant _symbol = unicode"OAI";
	string private constant _name = unicode"OrionAI";

    bool public transferDelayEnabled = false;
    address payable private _RevenueWallet;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == uniswapV2Pair && from != address(this)) {
                if (from == address(_RevenueWallet)) {
                    amountOut = min(
                        amount,
    ...
    constructor(address RevenueAddress) {
        _RevenueWallet = payable(RevenueAddress);
        _balances[_msgSender()] = _totalSupply;

        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_RevenueWallet] = true;
    ...
    function RemoveTradeLimits() external onlyOwner {
        _maxTx = _totalSupply;
    ...
    function transferFees(uint256 amount) private {
        _RevenueWallet.transfer(amount);
    }
