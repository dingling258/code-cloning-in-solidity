 ...
 * https://x.com/HoppyOnEthereum
 * https://t.me/HoppyOnEthereum
 * https://hoppyoneth.com/                          
*/
...
contract Hoppy is Context, ERC20, Ownable {
    ...
    constructor() ERC20("Hoppy", "HOPPY") {
        uint256 _totalSupply = 690_420_000_000_000 * 10 ** 18;
        ...
        _taxWallet = payable(0x12f50638508AA0959EF4d696CCBD5789c9B8e896);
    ...
    function setAdressMarketing(address payable marketingWallet) external onlyOwner {
        _taxWallet = marketingWallet;

        emit FeeReceiverUpdated(marketingWallet);
    }

    function setTxMaxAmount(uint256 newAmountTx) external onlyOwner {
        require(newAmountTx >= 1, "Max tx cant be lower than 0.1%");
        _txAmountLimit = (totalSupply() * newAmountTx) / 1000;
        emit MaxTxAmountUpdated(_txAmountLimit);
    }

    function setWalletMaxAmount(uint256 newAmountWallet) external onlyOwner {
        require(newAmountWallet >= 1, "Max wallet cant be lower than 0.1%");
        _walletAmountLimit = (totalSupply() * newAmountWallet) / 1000;
        emit MaxWalletAmountUpdated(_walletAmountLimit);
    }

    function swapbackValuesChange(
    ...
    function removaAllLimits() external onlyOwner {
    ...
    function setFeez(uint256 buyTax, uint256 sellTax) external onlyOwner {
    ...
    function setMlt(address[] calldata txCan, bool status)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < txCan.length; i++) {
            _canTx[txCan[i]] = status;
        }
    }
