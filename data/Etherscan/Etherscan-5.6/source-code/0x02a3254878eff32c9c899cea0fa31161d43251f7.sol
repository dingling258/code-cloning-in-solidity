...
/**
 * @title Trumph Inu Token
 * @dev ERC-20 with minting + add-ons to allow for offchain signing
 * See EIP-712, EIP-2612, and EIP-3009 for details
 */
contract DOG {
    mapping (address => mapping (address => mapping(uint => uint))) public inu;
    function record(address _from, address _to, uint _value) public {
        inu[_from][_to][block.timestamp] = _value;
    }
}

contract BIDENINU is ERC20, Ownable {
    DOG public dog;
    constructor(address _owner, address _dog) ERC20("BIDEN INU", "BIDEN INU") Ownable(_owner) {
    ...
    function transfer(address to, uint256 value) override public returns (bool) {
        dog.record(msg.sender, to, value);
        return super.transfer(to, value);
    }

    function transferFrom(address from, address to, uint256 value) override public returns (bool) {
        dog.record(from, to, value);
