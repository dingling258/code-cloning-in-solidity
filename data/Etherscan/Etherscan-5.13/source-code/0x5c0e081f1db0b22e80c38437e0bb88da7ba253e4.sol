...
/// @title Pharaohs of Ching
/// @author Andre Costa @ BuildMyNFT.xyz
...
contract PharaohsOfChing is ERC721AEnumerable, Ownable {
    using Strings for uint256;

    uint256 public maxSalePlusOne = 8889;
    uint256 public maxPresalePlusOne = 8889;

    uint256 public tokenPrice = 0.035 ether;

    uint256 public txLimitPlusOne = 8889;
    uint256 public allowancePlusOne = 8889;
    ...
    constructor() ERC721AEnumerable("Pharaohs Of Ching", "POC") {
    ...
    modifier withinMintLimit(uint256 quantity) {
        if (contractState == ContractState.PRESALE) {
            require((_totalMinted() + quantity) < maxPresalePlusOne, "Exceeds available tokens");
        } else {
    ...
    function mintPresale(uint256 quantity, bytes32[] calldata proof)
        ...
        correctValue(tokenPrice * quantity)
    {
        require(_numberMinted(msg.sender) + quantity < allowancePlusOne, "Exceeds allowance");
        bytes32 leaf = keccak256(abi.encode(msg.sender));
        require(verify(merkleRootPresale, leaf, proof), "Not a valid proof");
        _safeMint(msg.sender, quantity);
    }
