...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    modifier onlyDeveloper{
        require(msg.sender==developer,"Only Developer Function");_;
    }
    ...
    function setConfig(uint16 _version, uint16 _chainId, uint _configType, bytes calldata _config) external override onlyDeveloper {
        lzEndpoint.setConfig(_version, _chainId, _configType, _config);
    }

    function setSendVersion(uint16 _version) external override onlyDeveloper {
        lzEndpoint.setSendVersion(_version);
    }

    function setReceiveVersion(uint16 _version) external override onlyDeveloper {
        lzEndpoint.setReceiveVersion(_version);
    }

    function forceResumeReceive(uint16 _srcChainId, bytes calldata _srcAddress) external override onlyDeveloper {
    ...
    function setPrecrime(address _precrime) external onlyDeveloper {
        precrime = _precrime;
...
abstract contract NonblockingLzApp is LzApp {
    ...
    constructor(address _endpoint,address _dev) LzApp(_endpoint,_dev) {}
...
abstract contract TokenBridgeBase is NonblockingLzApp, ReentrancyGuard {
    ...
    constructor(address _endpoint,address _dev) NonblockingLzApp(_endpoint,_dev) {}
...
contract OriginalTokenBridge is TokenBridgeBase {
    ...
    constructor(address _endpoint, uint16 _remoteChainId, address _weth,address _dev) TokenBridgeBase(_endpoint,_dev) {
