...
// Root file: src/FabricaValidatorRegistry.sol
...
contract FabricaValidatorRegistry is IFabricaValidatorRegistry, Initializable, ContextUpgradeable, IERC165Upgradeable, ERC165Upgradeable, OwnableUpgradeable, PausableUpgradeable, FabricaUUPSUpgradeable {
    ...
    function addName(address addr, string memory name_) public onlyOwner whenNotPaused {
    ...
    function removeName(address addr) public onlyOwner whenNotPaused {
        require(bytes(_names[addr]).length > 0, "Validator name record for addr does not exist");
        delete _names[addr];
        emit ValidatorNameUpdated(addr, "");
    }

    function updateName(address addr, string memory name_) public onlyOwner whenNotPaused {
        require(bytes(_names[addr]).length > 0, "Validator name record for addr does not exist");
