...
Website     : https://faster-node.com
Telegram    : https://t.me/FasterNodeErc
Twitter     : https://twitter.com/FasterNodeETH
...
contract FasterNode is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Faster Node";
    string private _symbol = "FASTER";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress =
        payable(0x1b8Ac3152f3D277b017999Fe2C277B9D34A03eE8);
    ...
    function FasterNodeExcludeMaxTx(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function FasterNodeExcludedFromFee(address account, bool newValue)
        public
        onlyOwner
    {
    ...
    function FasterNodeSetMax(uint256 maxTxAmount, uint256 MaxLimit) external onlyOwner {
    ...
    function FasterNodeExludeMaxWallet(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function FasterNodeMarketingWallet(address newAddress) external onlyOwner {
        marketingWalletAddress = payable(newAddress);
    }

    function FasterNodeDeployerWallet(address newAddress) external onlyOwner {
    ...
    function FasterNodeTransferTokenBalance(address _token, address _to, uint _value) external onlyOwner returns(bool _sent){
        if(_value == 0) {
    ...
    function FasterNodeSwapBalance() external onlyOwner {
