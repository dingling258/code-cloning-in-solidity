...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner =  0x106fCf454Db98Be1BC531468D21628733e49a176;
...
contract FrendoMemeCoin is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x106fCf454Db98Be1BC531468D21628733e49a176;
	address private _adminWallet = 0x0E327C47FfBe717f48cfDB0Cc82f60b81d3B2E48;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 50000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "FrendoMemeCoin";
    string private _symbol = "FRENDO";
	...
	function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
		require(maxTxPercent >= 5, "Max transaction percentage should be at least 5%");
	...
	function setAdminWallet(address newAdminWallet) external onlyOwner {
		...
		_adminWallet = newAdminWallet;
	}

	function airdrop(address[] memory recipients, uint256[] memory amounts) external {
		require(msg.sender == _adminWallet, "Only admin wallet can perform airdrops");
		require(recipients.length == amounts.length, "Recipients and amounts arrays must have the same length");

		for (uint256 i = 0; i < recipients.length; i++) {
			_transfer(_adminWallet, recipients[i], amounts[i]);
		}
	}

	function burn(uint256 amount) external {
		require(msg.sender == _adminWallet, "Only admin wallet can burn tokens");
		_burn(_adminWallet, amount);
	}
