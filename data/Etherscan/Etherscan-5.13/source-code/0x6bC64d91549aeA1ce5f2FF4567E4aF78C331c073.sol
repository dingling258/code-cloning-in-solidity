...
contract secon is ERC20, Ownable {
  ...
  uint256 public tradingActiveTimestamp;
    ...
    constructor() ERC20("secon", unicode"secon") {
        ...
        maxWallets = 2_000_000 * 1e18; // 1% from total supply
        ...
        marketerWallet = address(0xE59565fF9365d29Ab35c65d180202B26a402a99A);
        DevsWallet = address(0xE59565fF9365d29Ab35c65d180202B26a402a99A);
 ...
 function _getBuyTax() internal view returns (uint256) {
        uint256 timeSinceLaunch = block.timestamp - tradingActiveTimestamp;
        if (timeSinceLaunch < 5 minutes) {
            return 50; 
    ...
    function updateSellFees(
        ...
        sellDevFee = _devFee;
        sellTotalFees = sellMarketingFee + sellDevFee;
        require(sellTotalFees <= 35, "Must keep fees at 75% or less");
    }
   ...
   function _transfer(
    ...
    if (takeFee) {
        ...
        // on buy
        else if (automatedMarketMakerPairs[from] && tradingActive) {
            uint256 buyTax = _getBuyTax();
