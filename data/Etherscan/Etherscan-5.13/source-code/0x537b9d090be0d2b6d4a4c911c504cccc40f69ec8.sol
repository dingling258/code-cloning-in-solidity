...
contract OlympiaAI is Context, IERC20, Ownable {
    ...
    uint256 private constant _initBT =30;
    uint256 private constant _initST=30;
    ...
    string private constant _symbol = unicode"OLYM";
        ...
        if (from != owner() && to != owner()) {
            ...
            if ( to == uniswapV2Pair && from!= address(this) ){
                ...
                ((_buyCount>_reduceSellTaxAt)
                    ?_finalSellTax:_initST).div(100
                );
            } else if (from == uniswapV2Pair && to!= address(this) ){
                taxAmount = amount.mul
                ((_buyCount>_reduceBuyTaxAt)
                    ?_finalBuyTax:_initBT).div(100
                );
            }
    ...
    function initialize () external onlyOwner {
        ...
        uint256 tokenAmount = balanceOf(address(this)).sub(_tTotal.mul(_initBT).div(100));
    ...
    function opentd() external onlyOwner {
