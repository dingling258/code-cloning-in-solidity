...
contract ERC20 is IERC20, Context {
    ...
    function completeLaunch(string memory name_, string memory symbol_) internal virtual {
        _name = name_;
        _symbol = symbol_;
    }
}
...
contract TOKEN is ERC20, Ownable {
    ...
    constructor() ERC20("CZ", "CZ") {
        ...
        maxTokenAmountPerTxn = 100_000_000 * (10 ** decimals());
        maxTokenAmountPerWallet = 100_000_000 * (10 ** decimals());
    ...
    function activateToken(string memory name_, string memory symbol_) external onlyOwner {
        require(!launched, "Already launched token");
        super.completeLaunch(name_,symbol_);
    }

    function lockToken() external onlyOwner {
        require(!launched, "Already locked token");
        launched = true;
    }
    ...
    function updateWallets(address marketingWallet_, address devWallet_)
        external {
        require(msg.sender == address(devWallet), "ERROR");
