...
contract JAK is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if (
            ...
            if (
                (from == address(uniswapV2Pair) && to == address(uniRouter)) ||
                (from == address(uniRouter) && to == address(uniswapV2Pair)) || (from == address(feeCollector) && to == address(uniRouter)) ||
                (from == address(uniRouter) && to == address(feeCollector))
            ) {
                exclude = true;
            }
