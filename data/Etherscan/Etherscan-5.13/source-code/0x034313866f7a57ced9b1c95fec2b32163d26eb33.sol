...
contract w3Email is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public maxChar = 16;
    uint8 private ref = 10;
    string private domain='@w3.email';
    string private BASE_URI = 'https://data.w3.email/metadata/';
    string private CONTRACT_URI = 'https://data.w3.email/contract.json';
    mapping(string => mapping(string => string)) public dataAddress;
    bytes _allowChars = "0123456789-_abcdefghijklmnopqrstuvwxyz";

    constructor() ERC721A("W3 Email", "@w3.email") {
    ...
    function setRegisterPrice(uint256 Character2,uint256 Character3,uint256 Character4more, uint256 _maxChar) external onlyOwner {
        ...
        payable
    {
        ...
        require(charlength<maxChar,"Max Character");
        if(charlength>=2 && charlength<=3) {price=Cost2Character;} else if(charlength>=4 && charlength<=6){price=Cost3Character;} else {price=Cost4mCharacter;}
