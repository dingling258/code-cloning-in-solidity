...
contract mcboystake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accTokensPerShare; // Accumulated Tokens per share, times 1e9. See below.
    }
    ...
    constructor(
        ...
        stakingToken = IBEP20(0x18500bF205a788d88e9377e74817C4C5271416Cf);
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            accTokensPerShare = accTokensPerShare.add(tokenReward.mul(1e9).div(lpSupply));
        }
        return user.amount.mul(accTokensPerShare).div(1e9).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) internal {
        ...
        uint256 tokenReward = calculateNewRewards().mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokensPerShare = pool.accTokensPerShare.add(tokenReward.mul(1e9).div(lpSupply));
        pool.lastRewardTimestamp = block.timestamp;
    }
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTokensPerShare).div(1e9).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accTokensPerShare).div(1e9);
