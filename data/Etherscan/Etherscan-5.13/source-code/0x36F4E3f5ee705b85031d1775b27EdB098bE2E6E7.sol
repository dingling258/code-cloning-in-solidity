...
// File: contracts/DynastiesEco.sol
...
contract DynastiesEcosystemPresale is Ownable {
    IERC20 public token;
    ...
    /***
     * @dev buyTokenWithReferral funtion to buy tokens
     * @param _amount No of tokens to buy
     */
        ...
        payable
    {
        ...
        uint256 _totalPayAmount = _totalPayUsd / _ethToUsd;
        require(msg.value >= _totalPayAmount, "Not enough payment!");
        address _refereer = _referralAddress;
        uint256 _buyAmount = _amount;
        uint256 _weiAmount = _buyAmount * 1e18;
        uint256 _bonusAmount = ((_buyAmount * _bonus) / 100) * 1e18;
        uint256 _totalAmount = _weiAmount + _bonusAmount;
        uint256 _tokenDecimals = tokenMetadata.decimals();
        uint256 _subDecimals = 18 - _tokenDecimals;
        uint256 _totalTokenAmount = _totalAmount / (10**_subDecimals);
        uint256 _referralBonusAmount = ((_buyAmount * 10) / 100) * 1e18;
        uint256 _totalTokenReferrer = _referralBonusAmount / (10**_subDecimals);

        require(
            (totalSold + _totalTokenAmount + _totalTokenReferrer) <=
                presaleTokenAmount,
            "Presale token amount exceeds!"
        );
        ...
        bool successReferrer = token.transferFrom(
            sellerAddress,
            _refereer,
            _totalTokenReferrer
        );
        require(successReferrer, "Failed to transfer token to referrer!");

        //added to totalSold
        uint256 _totalSoldToken = _totalTokenAmount + _totalTokenReferrer;
        totalSold += _totalSoldToken;
    }
