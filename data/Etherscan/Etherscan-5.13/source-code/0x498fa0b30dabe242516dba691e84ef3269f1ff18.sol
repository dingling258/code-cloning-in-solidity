...
contract TaaSAI is Context, IERC20, Ownable {
    ...
    string private constant _name = "Taas AI";
    string private constant _symbol = "TAAS";
    ...
    event TaxSwapThresholdUpdated(uint256 taxSwapThreshold);
    ...
    function clearstuckETH(uint256 ethAmount) external onlyOwner {
    ...
    function changeTaxSwapThreshold(uint256 percent) external onlyOwner {
        require(percent <= 1000, "changeTaxSwapThreshold: Percentage should be less than or equal to 1000");
        _taxSwapThreshold = _tTotal * percent / 1000;
    ...
    function setBuyTaxSAI(uint256 tax) external onlyOwner {
        require(tax <= 45, "Tax should be less than or equal to 45");
        _buyTax = tax;
    }

    function setSellTaxSAI(uint256 tax) external onlyOwner {
        require(tax <= 45, "Tax should be less than or equal to 45");
        _sellTax = tax;
    }
    
    function setMaxTxSAI(uint256 percent) external onlyOwner {
        _maxTxSize = _tTotal * percent / 1000;
    }

    function setMaxWalletSAI(uint256 percent) external onlyOwner {
