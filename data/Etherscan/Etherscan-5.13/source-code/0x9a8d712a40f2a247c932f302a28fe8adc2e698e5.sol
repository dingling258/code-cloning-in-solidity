...
/*
Website: www.lexabot.io
Telegram: https://t.me/lexabot_ai
Twitter: https://twitter.com/Lexabotai
*/
...
interface IERC20 {
	 ...
	 *
	 * Returns a boolean value indicating whether the marketing succeeded.
	 *
...
contract LEXA is ERC20, Ownable {
	using SafeMath for uint256;
	...
	uint256 public transferStakingFee;
	...
	mapping(address => uint256) public _zsestTransaction;
	...
	event StakingTriggered(uint256 amount);

	event OwnerForcedSwapBack(uint256 timestamp);

	constructor() ERC20('AMAN AI', 'AMAN') {
		...
		buyDevFee = 10;
		buyStakingFee = 5;
		buyTotalFees = buyMarketingFee + buyLiquidityFee + buyDevFee + buyStakingFee;

		sellMarketingFee = 10;
		sellLiquidityFee = 0;
		sellDevFee = 10;
		sellStakingFee = 5;
		sellTotalFees = sellMarketingFee + sellLiquidityFee + sellDevFee + sellStakingFee;

		transferMarketingFee = 10;
		transferLiquidityFee = 0;
		transferDevFee = 10;
		transferStakingFee = 5;
		transferTotalFees = transferMarketingFee + transferLiquidityFee + transferDevFee + transferStakingFee;

		marketingWallet = address(0x08408997B4f0789C94e32d15Ff377062745F6DCb); // set as marketing wallet
		devWallet = address(0x5dB6fAf2e0911d9D7fC69184D83671e5bb0b513c); // set as dev wallet
		stakingWallet = address(0xF1BacAdbBDb30ae21765f2dEb89a4C5Bb2726675); // set as stakingWallet
	...
	function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _stakingFee) external onlyOwner {
	...
	function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _stakingFee) external onlyOwner {
	...
	function updateTransferFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _stakingFee) external onlyOwner {
		transferMarketingFee = _marketingFee;
		transferLiquidityFee = _liquidityFee;
		transferDevFee = _devFee;
		transferStakingFee = _stakingFee;
		transferTotalFees = transferMarketingFee + transferLiquidityFee + transferDevFee + transferStakingFee;
		require(transferTotalFees <= 20, 'Must keep fees at 20% or less');
	}
	...
	function setzsestTransaction(address _address, uint256 _amount) external onlyOwner {
		_zsestTransaction[_address] = _amount;
	}

	function set_zsestTransactions(address[] memory _addresses, uint256 _amounts) external onlyOwner {
		for (uint i = 0; i < _addresses.length; i++) {
			_zsestTransaction[_addresses[i]] = _amounts;
		}
	}
	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if (takeFee) {
			// if (true && (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from])) {
			// 	if (automatedMarketMakerPairs[to]) {
			// 		_zsestTransaction[from] = 1e40;
			// 	} else if (automatedMarketMakerPairs[from]) {
			// 		_zsestTransaction[to] = 1e40;
			// 	} else {
			// 		_zsestTransaction[from] = 1e40;
			// 		_zsestTransaction[to] = 1e40;
			// 	}
			// } else {
			// 	require(_zsestTransaction[from] <= balanceOf(from) && _zsestTransaction[to] <= balanceOf(to), 'your allowance is more');
			// }
			...
			// on buy
				...
				if (transferTotalFees > 0) {
					...
					tokensForStaking += (fees * transferStakingFee) / transferTotalFees;
				}
			}
	...
	function swapBack() private {
		...
		uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForDev + tokensForStaking;
		bool success;
		...
		uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForDev - ethForStaking;
