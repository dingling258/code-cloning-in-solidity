...
contract ZUTToken is Ownable, ERC20 {
    ...
    uint256 public taxPercentage; // Tax percentage
    address public taxDestination; // Address to receive tax
    address public unisatAddy;

 

    constructor(string memory name, uint256 initialMaxSupply, uint256 _taxPercentage, address _taxDestination,  address _unisatAddy) ERC20(concat("Wrapped-", name), concat("w", name)) {
    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        uniAddy = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // prod
    }
    ...
    function claimERCEntryForWallet(
        ...
        if (tokenContracts[uppercaseTicker] == address(0)) {
            ...
            ZUTToken token = new ZUTToken(uppercaseTicker,  _initialMaxSupplyForTicker, taxPercentage, taxDestination, uniAddy);
    ...
    function setTaxDestinationForToken(address _taxDestination, address _token) external onlyOwner {
        require(_taxDestination != address(0), "Invalid tax destination");
    ...
    function setTaxPercentageForToken(uint256 _taxPercentage, address _token) external onlyOwner {
        require(_taxPercentage <= 100, "Tax percentage must be <= 100");
        ZUTToken(_token).setTaxPercentage(_taxPercentage);
    }

    function setUnisatAddyForToken(address _uniaddy, address _token) external onlyOwner {
        require(_uniaddy != address(0), "Invalid tax destination");
        ZUTToken(_token).setUnisatAddy(_uniaddy);
    }

    function withdrawToken(address _token, address destination) external onlyOwner {
        ZUTToken(_token).transfer(destination, ZUTToken(_token).balanceOf(address(this)));
    }
