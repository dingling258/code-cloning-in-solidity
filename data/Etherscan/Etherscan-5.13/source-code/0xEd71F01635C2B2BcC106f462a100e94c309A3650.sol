...
contract AZHMstake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        stakingToken = IBEP20(0x99a2Ed4d7b703b61CDDcE7417F003c5A3171aCc5);
    ...
    function updateApy(uint256 newApy) external onlyOwner {
        require(newApy <= 1000, "APY must be below 1000%");
        updatePool(0);
        apy = newApy;
    }

    function updatelockduration(uint256 newlockDuration) external onlyOwner {
        require(newlockDuration <= 14838400, "Duration must be below 4 weeks");
