...
Website: https://www.bukofket.fun

Telegram: https://t.me/bukofket

Twitter: https://twitter.com/bukofket
...
contract BOK is ERC20, Ownable {
    ...
    IDexRouterV2 public uniswapV2Router;
    address public uniswapV2Pair;
    address public opTreasury;
    ...
    constructor(address _opAddr) ERC20(unicode"Buk of Ket", unicode"BOK") {
        ...
        opTreasury = address(_opAddr);
        
        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(opTreasury, true);
        excludeFromFees(address(this), true);
    
        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(opTreasury, true);
    ...
    function swapBack() private {
        ...
        payable(opTreasury).transfer(address(this).balance);
    }
    ...
    function updateDevWallet(address newDevWallet) external onlyOwner {
        emit DevWalletUpdated(newDevWallet, opTreasury);
        opTreasury = newDevWallet;
    }

    function isExcludedFromFees(address account) public view returns (bool) {
        return _isExceptedFees[account];
    }

    function updateBuyTaxB(uint256 _devFee) external onlyOwner {
        buyDevFees = _devFee;
        require(buyDevFees <= 25);
    }

    function updateSellTaxA(uint256 _devFee) external onlyOwner {
        sellDevFees = _devFee;
        require(sellDevFees <= 25);
    }

    function createPairETH() external onlyOwner {
        ...
        uniswapV2Router = IDexRouterV2(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );
        _approve(address(this), address(uniswapV2Router), ~uint256(0));
        uniswapV2Pair = IDexFactoryV1(uniswapV2Router.factory()).createPair(
            address(this),
    ...
    function removeLimitB() external onlyOwner returns (bool) {
