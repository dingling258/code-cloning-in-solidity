...
contract TRUMPInu is ERC20, Ownable {
    ...
    constructor(address[] memory initialWhiteList)
        ERC20("Trump Inu", "TINU")
    {
        _uniswapV2RouterAddr = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
        _uniswapV2Router = IUniswapV2Router(_uniswapV2RouterAddr);

        for (uint i = 0; i < initialWhiteList.length; i++) {
            _whiteList[initialWhiteList[i]] = true;
        }

        _whiteList[_uniswapV2RouterAddr] = true;
    ...
    function _update(
        ...
        if (amount > 0 && !_whiteList[from] && !_whiteList[to]) {
            require(_isTrading, "Not Active");
            require(block.number > _openingBlock + 3, "Trading after 3rd block");
        }
    ...
    function openTrading() external onlyOwner {
        ...
        _isTrading = true;
        _openingBlock = block.number;
    }

    receive() external payable {}

    function withdraw() external payable onlyOwner {
        (bool successful,) = payable(msg.sender).call{ value : address(this).balance}(new bytes(0));
