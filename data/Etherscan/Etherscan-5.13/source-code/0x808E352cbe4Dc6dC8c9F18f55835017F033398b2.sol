...
contract Trumpump is ERC20, Ownable {
    ...
    string private constant _name = "Trump & Pump";
    string private constant _symbol = "TRUMPUMP";

    uint256 public initialTotalSupply = 100_000_000 * 1e18;
    uint256 public maxTransactionAmount = 2_000_000 * 1e18; // 2%
    uint256 public maxWallet = 2_000_000 * 1e18; // 2%
    uint256 public swapTokensAtAmount = 50_000 * 1e18; // 0.05%
    ...
    constructor() ERC20(_name, _symbol) {
        ...
        marketingWallet = payable(0xA87826C429281Cab3719beA4C1Ecbaf6fc57aeC4);     
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[to]) {
                ...
                if (automatedMarketMakerPairs[from]) {
                    ...
                    if (toBoost.buy == 0) {
                        toBoost.buy = (buyCount < 20) ? (block.timestamp - 1) : block.timestamp;
                    }
                } else {
                    BoostInfo storage fromBoost = boostInfo[from];
