...
// File contracts/interfaces/INovaFrontierCreator.sol

// License-Identifier: MIT
pragma solidity ^0.8.9;

interface INovaFrontierCreator {
    function funcGetCreator(uint256 _id) external view returns (address);

    function funcSetCreator(uint256 _id, address _creator) external;
}
...
interface IMintableNFT {
    function funcMint(address _pTo, uint256 _pId) external; /* onlyRole(MINTER_ROLE) */
}


// File contracts/interfaces/IBurnableNFT.sol

// License-Identifier: MIT
pragma solidity ^0.8.9;

interface IBurnableNFT {
    function funcBurn(uint256 _pId) external; /* onlyRole(BURNER_ROLE) || NFT owner */

    function funcBatchBurn(uint256 _pFromId, uint256 _pToId) external; /* onlyRole(BURNER_ROLE) */
}


// File contracts/interfaces/INovaFrontierNFT.sol
...
interface INovaFrontierNFT is IMintableNFT, IBurnableNFT {
...
// File contracts/NovaFrontierNFT.sol
...
{
    /*
    function initialize(string memory _pUri) public initializer {
        __NFT_init("NovaFrontierNFT", "NOVF", _pUri);
    }*/

    constructor(string memory _pUri) {
        __NFT_init("NovaFrontierNFT", "NOVF", _pUri);
    }
    ...
    /// @notice Set new URI address
    /// @param _pUri URI Address
    function funSetURI(string memory _pUri) external onlyRole(EDITOR_ROLE) {
        uri = _pUri;
        emit SetURI(_pUri);
    }

    /// @notice Enable Opensea address or not
    /// @param _pProxyRegistryAddress  ProxyAddress
    /// @param _pIsOpenSeaProxyActive Opensea enable or disabled
    ...
    /// @notice Transfer multi NFT to one owner
    /// @param _pTo NFT owner address
    /// @param _pIds NFT Ids
    ...
    /// @notice Revoke NFT Ownership of owner
    /// @param _pOwner NFT By owner
    /// @param _pIds NFT Ids to transsfer
    ...
    /// @notice NFT Mint only from role address
    /// @param _pTo NFT owner
    /// @param _pId NFT Id
    function funcMint(
        address _pTo,
        uint256 _pId
    ...
    /// @notice             Bunr NFT only from owner or burner_role
    /// @param _pId         NFT Id
    /// @dev                Error permission TIPSYEC_405
    ...
    /// @notice             Burn multi nft item from smartcontract owner
    /// @param _pFromId     NFT Id begin
    /// @param _pToId       NFT Id end
    ...
    */
    ...
    /// @notice             Set creator member on list
    /// @param _pId         Creator ID
    /// @param _pAccount    Creator Address
    ...
    /// @notice             Get NFT Onwer time
    /// @param _pId         NFT Id
    /// @return _           Time use of owner
    ...
    /// @notice             Transfer NFT from user to user
    /// @dev                See {IERC721-isApprovedForAll}.
    /// @param _pFrom       From user
    /// @param _pTo         To user
    /// @param _pTokenId    NFT ID
    function _transfer(
        address _pFrom,
        address _pTo,
        uint256 _pTokenId
    ...
    /// @notice             Mint NFT and store information inside creators, save timestamp from mint time Emits a {Transfer} event.
    /// @param _pTo         To user
    /// @param _pTokenId    NFT ID
    ...
    /// @notice             Burn single NFT and revoke own time
    /// @param _pTokenId    NFT ID
