/**
 /*
//  ________  ________  _______   _______   __      __        _______   ________   ______   _______                          __    ________  _______    ______  
// /        |/        |/       \ /       \ /  \    /  |      /       \ /        | /      \ /       \                       _/  |_ /        |/       \  /      \ 
// $$$$$$$$/ $$$$$$$$/ $$$$$$$  |$$$$$$$  |$$  \  /$$/       $$$$$$$  |$$$$$$$$/ /$$$$$$  |$$$$$$$  |                     / $$   \$$$$$$$$/ $$$$$$$  |/$$$$$$  |
...
//                                                                                                                        $$$$$$/                               
//                                                                                                                          $$/                                 
The next $BOME killer is a Bear?! Potential 100x?
You missed $PIPI $BOME $SLERF….
Don’t Miss $TBC

Web: https://TBCLAND.com

Telegram: https://t.me/TeddyBearCoins

Twitter: https://twitter.com/TeddyBearCoins

Instagram: https://www.instagram.com/teddybearcoins/
...
contract TEDDYBEAR_LootBox is OwnerWithdrawable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function getTokenAmount(
        ...
        uint256 amtOut;
        if (token != address(0)) {
            require(payableTokens[token] == true, "Error: Token not allowed");
    ...
    function getPayAmount(
        ...
        if (token != address(0)) {
            require(payableTokens[token] == true, "Error: Token not allowed");
            uint256 price = tokenPrices[token];
            amtOut = amount.mul(price).div(10 ** saleTokenDec);
        } else {
            amtOut = amount.mul(rate).div(10 ** saleTokenDec);
        }
        return amtOut;
    }
    ...
    function buyLootBox(
        ...
        uint256 saleTokenAmt = lootBoxes[lootBoxIndex].amount;
        uint256 discountAmt = saleTokenAmt
            .mul(lootBoxes[lootBoxIndex].discount)
            .div(100);

        uint256 payAmount = getPayAmount(token, saleTokenAmt.sub(discountAmt));

        if (token != address(0)) {
            require(
                IERC20(token).allowance(msg.sender, address(this)) >= payAmount,
                "Error: Check the token allowance"
            );

            IERC20(token).safeTransferFrom(msg.sender, owner(), payAmount);
        } else {
            require(msg.value >= payAmount, "Error: Not enough ETH");
            payable(owner()).transfer(payAmount);

            if (msg.value.sub(payAmount) > 0) {
                payable(msg.sender).transfer(msg.value.sub(payAmount));
            }
        }
