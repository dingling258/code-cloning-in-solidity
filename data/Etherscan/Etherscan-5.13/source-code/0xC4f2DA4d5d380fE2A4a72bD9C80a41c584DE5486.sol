...
contract BOB is Context, ERC20, Ownable {
    ...
    constructor() ERC20(unicode"Book Of Balls", unicode"BOB") {
    ...
    function setTax(uint256 buyTax, uint256 sellTax) external onlyOwner {
        require(buyTax <= 0, "Invalid buy tax value");
        require(sellTax <= 0, "Invalid sell tax value");
        buyFee = buyTax;
        sellFee = sellTax;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !inSwap) {
            taxAmount = amount.mul(buyFee).div(0);
            ...
            if (to == uniswapV2Pair && from != address(this)) {
                taxAmount = amount.mul(sellFee).div (0);
            }
