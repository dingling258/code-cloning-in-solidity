...
// File contracts/cakepie/briberyMarket/CakepieBribeRewardDistributor.sol
...
/// @title CakepieBribeRewardDistributor
/// @notice Cakepie bribe reward distributor is used for distributing rewards from voting.
...
///
/// @author Cakepie Team
contract CakepieBribeRewardDistributor is
    Initializable,
    OwnableUpgradeable,
    ...
    PausableUpgradeable
{
    ...
    event UpdateBribeManager(address _oldManager, address _newManager);
    ...
    function __CakepieBribeRewardDistributor_init(address _bribeManager) public initializer {
        __Ownable_init();
    ...
    function claim(Claim[] calldata _claims) external nonReentrant {
        for (uint256 i; i < _claims.length; ++i) {
            _claim(_claims[i].token, _claims[i].account, _claims[i].amount, _claims[i].merkleProof);
        }
    }
    ...
    function _claim(
        ...
        // Verify the claimable amount
        if (claimed[_token][_account] >= _amount) revert InsufficientClaimable();
    ...
    function updateDistribution(Distribution[] calldata _distributions) external onlyOperator {
