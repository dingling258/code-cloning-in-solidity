...
// File contracts/DeFiSmartTransfer.sol
...
contract DeFiSmartTransfer is Operator {
    ...
    struct NativeTransfer {
        ...
        uint amount; // Amount of tokens to be transferred
        uint fee; // Transfer fee amount
        address payable feeReceiver; // Address to receive the transfer fee
    ...
    // Event to be emitted when a transfer is completed
    ...
    function executeNativeTransfer(
        ...
        if ((transfer.fee < 0) || (transfer.fee > transfer.amount)) {
        ...
        if (
            ...
            (msg.sender == transfer.feeReceiver)
        ) {
        ...
        (success, ) = transfer.feeReceiver.call{value: transfer.fee}("");
        if (!success) {
            revert NativeTransferFailed({
                receiver: transfer.feeReceiver,
                amount: transfer.fee
            });
        }
    ...
    function executeErc20Transfer(
        ...
        // Transfer the fee from the payer to the fee receiver
        IERC20(transfer.token).safeTransferFrom(
            transfer.payer,
