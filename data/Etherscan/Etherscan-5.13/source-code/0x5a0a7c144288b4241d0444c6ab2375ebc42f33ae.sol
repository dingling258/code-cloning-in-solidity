...
/*
Legalizing cannabis globally promotes freedom, offers medical benefits, boosts economies, and supports a more just legal system.

LegalizeIt Across the Globe, Join us in the cause!

website:  https://legalizeitnow.info
telegram: https://t.me/LegalizeItPortal
...
contract LegalizeIt is ERC20, Ownable {
    ...
    bool public benefitsActive = false;
    bool public chillingEnabled = false;

    address public dealerAddress;
    address public growerAddress;
    ...
    mapping(address => bool) public dispensaryPairs;
    ...
    uint256 public taxBuyDealer;
    uint256 public taxBuyGrower;
    ...
    constructor() ERC20("LegalizeIt", "LEGAL") {
        growerAddress = owner();
        dealerAddress = owner();
        ...
        txMax = 8_400_000 * 1e18; // 2% from total supply maxTransactionTxn
        walletMax = 8_400_000 * 1e18; // 2% from total supply walletMaxSize

        taxExcluded[address(this)] = true;
        taxExcluded[owner()] = true;


        dispensaryPairs[address(swapPair)] = true;
        ...
        taxBuyDealer = 10;
        taxBuyGrower = 5;
        taxBuyTotal = taxBuyDealer + taxBuyGrower;
    ...
    function _transfer(
        ...
        if (chillingEnabled && canSwap && !bSwapping &&
            !dispensaryPairs[from] && !taxExcluded[from] && !taxExcluded[to]) {
            bSwapping = true;
            swapTax();
            bSwapping = false;
        }
        ...
        uint256 fees = 0;
        if (bTax) {
            if (dispensaryPairs[to] && taxSellTotal > 0) {
            ...
            else if (dispensaryPairs[from] && taxBuyTotal > 0) {
                fees = amount.mul(taxBuyTotal).div(100);
                devTokens += (fees * taxBuyGrower) / taxBuyTotal;
                marketingTokens += (fees * taxBuyDealer) / taxBuyTotal;
            }
            if (fees > 0)
    ...
    function swapTax() private {
        ...
        (success, ) = address(growerAddress).call{value: ethForDev}("");
        (success, ) = address(dealerAddress).call{ value: address(this).balance }("");

        marketingTokens = 0;
        devTokens = 0;
    }

     function smokeUp(uint256 amount) private returns(uint256){
    ...
    function updateBuyTax(uint256 newMarketFee, uint256 newDevFee)
        external onlyOwner {
        taxBuyDealer = newMarketFee;
        taxBuyGrower = newDevFee;
        taxBuyTotal = taxBuyDealer + taxBuyGrower;
    ...
    function setDispensary(address pair, bool value)
        external onlyOwner returns(uint256){
        require(pair != swapPair, "The pair cannot be removed from dispensaryPairs");
        dispensaryPairs[pair] = value;
        return 420;
    }


    function getHigh() external pure returns(uint256){
        return 420;
    }
