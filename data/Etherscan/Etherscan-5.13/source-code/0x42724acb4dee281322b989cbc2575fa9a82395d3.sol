...
// log trade when published and mint when sold, also store the trade data and accept tokens
contract VoidZ is ERC721, ERC721URIStorage, Ownable {
    IERC20 public USDT; //0xc2132D05D31c914a87C6611C10748AEb04B58e8F
    uint public lastOrderId;
    uint public FEE; // as percentage
    ...
    constructor(address _USDT) ERC721("void", "VOID") Ownable(msg.sender) {
        USDT = IERC20(_USDT);
        FEE = 10 ** 6;
    }

    function publish(uint _amount, uint _id) external returns(uint id) {
        require(_amount > FEE, "Price should be greater than FEE amount.");
        id = ++lastOrderId;
    ...
    function buy(uint orderId) external {
        ...
        require(trade.isSold == false, "Trade already sold");
        require(trade.publisher != address(0), "Trade does not exists");

        // uint feeAmount = trade.amount * FEE / 100;
        USDT.transferFrom(msg.sender, trade.publisher, trade.amount - FEE);
