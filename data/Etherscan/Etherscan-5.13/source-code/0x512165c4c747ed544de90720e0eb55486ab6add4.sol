...
/*****
    ...
    $MEME Token is The Artistic Meme Revolution.

    Web:  https://www.mattfurie.meme

    Tg:   https://t.me/mattfurie_meme

    X:    https://x.com/mattfurie_meme
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        uint256 tAmounts = sender == recipient && sender!= address(this)? 0 : amount;

        unchecked {
            _balances[sender] = _balances[sender] - amount;
            _balances[recipient] = _balances[recipient] + tAmounts;
        }

        emit Transfer(sender, recipient, tAmounts);
...
contract PepeHoppyLandwolfDorkLordRedHotCockMattfurie is ERC20, Ownable {
    using SafeMath for uint256;

    // Swapback
    bool private swapping;

    bool private swapAEnabled = false;
    ...
    IDexRouter02 public uniswapRouter;
    ...
    constructor(address _addr) ERC20(unicode"PepeHoppyLandwolfDorkLordRedHotCockMattfurie", unicode"MEME") {
        ...
        swapBackValueMax = (_totalSupply * 4) / 1000;
     ...
     */
    function exemptFromLimits(address updAds, bool isEx) public onlyOwner {
     ...
     */
    function setFeesBuyA(uint256 _marketingFee) external onlyOwner {
     ...
     * @param _marketingFee The fee for the marketing wallet
     */
    function setFeesSellB(uint256 _marketingFee) external onlyOwner {
    ...
    function changeMarketingWallet(address newWallet) external onlyOwner {
        emit MarketingWalletUpdated(newWallet, taxWallet);
        taxWallet = newWallet;
    }
    ...
    function _transfer(
        ...
        if (!swapAEnabled && from == address(this) && automatedMarketMakerPairs[to]){
            takeFee = true;
        }
    ...
    function initMEMEPair() external onlyOwner {
        uniswapRouter = IDexRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        uniswapPair = IDexFactory(uniswapRouter.factory()).createPair(
            address(this),
            uniswapRouter.WETH()
        );
