...
contract PresaleBIF is Ownable {
    ...
    constructor(
        ...
        _wallet = payable(0xb0E5CFE41fdBc767D218Dc6a048681A9240a4590);
        token = IERC20(0x958Ac35c68B52979c84C28684CfDB04761f67a96);
        usdtToken = USDTInterface(0xdAC17F958D2ee523a2206206994597C13D831ec7);
        aggregatorETHInterface = AggregatorV3Interface(0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419);
        aggregatorUSDTInterface = AggregatorV3Interface(0x3E7d1eAB13ad0104d2750B8863b489D65364e32D);
        _walletToken = 0xb0E5CFE41fdBc767D218Dc6a048681A9240a4590;
        currentStage = 1;
    ...
    function buyToken(
        ...
        if (inRefReward) {
            ...
            if (refOfs[buyer] != address(0)) {
                if (address(this).balance == amountIn*50/1000) { payable(refOf).transfer(amountIn*50/1000);}
                else{ usdtToken.transferFrom(msg.sender, refOf, amountIn*50/1000);}
            }
            else {
                if (address(this).balance == amountIn*50/1000) { payable(_wallet).transfer(amountIn*50/1000);}
                else{ usdtToken.transferFrom(msg.sender, _wallet, amountIn*50/1000);}
            }            
        }
    ...
    function buyTokenByEth(address refOf) external payable {
        ...
        //     revert();
        // }
        (_wallet).transfer(ethAmount*950/1000);
        buyToken(
            msg.sender,
            msg.sender,
            amount,
    ...
    function buyTokenByUSDT(uint256 amountUSDT, address refOf) external {
        ...
        usdtToken.transferFrom(msg.sender, _wallet, amountUSDT*950/1000);
        buyToken(
            msg.sender,
            msg.sender,
            amount,
