...
contract VitalikStaking is ReentrancyGuard, DividendTracker, Ownable {

    IERC20 public immutable vitalikToken;
    ...
    constructor(address _vitalikToken) {
        require(_vitalikToken != address(0), "cannot be 0 address");
        vitalikToken = IERC20(_vitalikToken);

        //@dev initialize staking periods and boosts
        stakingPeriodsInDays.add(90);
        stakingPeriodsInDays.add(180);
        stakingPeriodsInDays.add(365);
        stakingPeriodBoost[90] = 30;
        stakingPeriodBoost[180] = 60;
        stakingPeriodBoost[365] = 120;
    ...
    function setBlacklisted(address[] memory _addresses, bool _blacklisted) external onlyOwner {
        ...
        if(tokensToTransfer > 0){
            vitalikToken.transfer(address(owner()), tokensToTransfer);
        }
    }
    ...
    function revokeAllTokens() external onlyOwner {
        ...
        vitalikToken.transfer(address(owner()), vitalikToken.balanceOf(address(this)));
        for(uint256 i = 0; i < _addresses.length; i++){
            address addy = _addresses[i];
            User storage user = users[addy];
            if(user.withdrawableTokens > 0){
    ...
    function deposit(uint256 _amount, uint48 _stakingDurationInDays) external nonReentrant {
        ...
        uint112 initialBalance = uint112(vitalikToken.balanceOf(address(this)));
        vitalikToken.transferFrom(address(msg.sender), address(this), _amount);
        amountTransferred = uint112(vitalikToken.balanceOf(address(this)) - initialBalance);
    ...
    function withdrawTokens() external nonReentrant {
        ...
        vitalikToken.transfer(address(msg.sender), amount);
    ...
    function emergencyWithdrawTokens() external nonReentrant {
        ...
        vitalikToken.transfer(address(msg.sender), amountForUser);
        if(amountForPenalty > 0){
            vitalikToken.transfer(address(owner()), amountForPenalty);
        }
    ...
    function compound(uint256 minOutput) external nonReentrant {
        ...
        if(amountEthForCompound > 0){
            uint256 initialBalance = vitalikToken.balanceOf(address(this));
            buyBackTokens(amountEthForCompound, minOutput);
            uint112 amountTransferred = uint112(vitalikToken.balanceOf(address(this)) - initialBalance);
            user.baseTokensStaked += amountTransferred;
            setInternalBalance(msg.sender, user);
        } else {
            revert("No rewards");
