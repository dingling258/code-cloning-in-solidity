...
// File: contracts/transactionLogger.sol
...
contract DigitalCurrencyLLCContract is ReentrancyGuard {
    using SafeERC20 for IERC20;

    string public constant companyName = "Digital Currency LLC";
    uint256 public constant companyIdentificationNumber = 445643849;
    string public documentLink = "ipfs link to the company's legal documents";
    string public name = "Digital Currency LLC Contract"; // Public name tag

    event DepositReceived(address indexed from, uint256 amount, string currency);
    event Withdrawal(address indexed token, address indexed from, address indexed to, uint256 amount);
    event ExecuteTransaction(address indexed owner, address indexed to, uint256 value, bytes data);
    event OwnerConfirmTransaction(address indexed owner, uint256 txIndex);
    ...
    receive() external payable {
        emit DepositReceived(msg.sender, msg.value, "ETH");
    }
        ...
        notExecuted(_txIndex)
    {
        ...
        require(count >= required, "Cannot execute transaction");

        transaction.executed = true;
        (bool success, ) = transaction.to.call{value: transaction.value}(transaction.data);
        require(success, "Transaction failed");

        emit ExecuteTransaction(msg.sender, transaction.to, transaction.value, transaction.data);
    }
    ...
    // Withdraw ERC20 tokens using a multi-sig transaction
    function withdrawToken(IERC20 token, address to, uint256 amount) public onlyOwner {
        ...
        submitTransaction(address(token), 0, data);
    }
}