...
contract MuseAI is Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping(address => uint256) private _balance;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromMuseFee;
    ...
    address payable public museFee;
        
    uint256 private launchedAt;
    uint256 private launchDelay = 2;
    bool private launch = false;

    uint256 private constant minMuseSwap = onePercent / 2550; //0.04% from Liquidity supply
    ...
    constructor() {       
        museFee = payable(0x059bC29f8e80AA1f965B86a7dA0852e8b5FfA25d);

        _isExcludedFromMuseFee[msg.sender] = true;
        _isExcludedFromMuseFee[address(this)] = true;       
        _isExcludedFromMuseFee[museFee] = true;
    ...
    function createMusePairs() external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_isExcludedFromMuseFee[from]) {
            _museTax = 0;
            _basicTransfer(from, to, amount, false); return;
        } else {
            require(launch, "Wait till Muse Launch!");

            if (block.number < launchedAt + launchDelay) {_museTax=49;} else {
                if (from == uniswapV2Pair) {
                    require(balanceOf(to) + amount <= maxWalletAmount, "Max wallet 2% at launch");
                    _museTax = buyTax;
                } else if (to == uniswapV2Pair) {
                    uint256 tokensToSwap = balanceOf(address(this));
                    if (tokensToSwap > minMuseSwap && !inSwapAndLiquify) {
                        if (tokensToSwap > onePercent) {
                            tokensToSwap = onePercent;
                        }
                        if(amount > minMuseSwap)
                            swapTokensForEth(tokensToSwap);
                    }
                    _museTax = sellTax;
                } else {
                    _museTax = transferTax;
                }
            }
        }

        uint256 taxTokens = (amount * _museTax) / 100;
    ...
    function removeMuseLimits() external onlyOwner {
        maxWalletAmount = _totalSupply;
    }

    function newMuseTax(uint256 newBuyTax, uint256 newSellTax) external onlyOwner {
        buyTax = newBuyTax;
        sellTax = newSellTax;
    }

    function sendEthBalances(uint256 amount) private {
        museFee.transfer(amount);
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        museFee.transfer(ethForMarketing);
    }
