...
abstract contract AntiBotHelper is Ownable {
    ...
    event AccountBlacklisted(address account, bool flag);
    ...
    function _initializeAntiBot(bytes memory param) internal {
        (
            ...
            bool antiSniperOn,
            bool blacklistOn
        ) = abi.decode(param, (uint16, uint16, uint16, bool, bool));
    ...
    /**
     * @notice Blacklist account
     */
    function batchBlacklist(
        address[] calldata accounts,
        bool flag
    ) external onlyOwner {
        require(_isBlacklistOn, "blacklist add-on disabled");
        uint256 len = accounts.length;
        for (uint256 i; i < len; ) {
            _blacklist[accounts[i]] = flag;
            emit AccountBlacklisted(accounts[i], flag);
            unchecked {
                ++i;
            }
        }
    ...
    /**
     * @notice Check if the given account is blacklisted
     */
...
contract ERC20ByBankPad is ERC20ByBankPadBase {
    bytes32 public constant bankUUIDHash =
        0x11647894f3c5bda261d91e16c88c55f44e8b0d2a7bee92a98cf0ba24d00980af;
    ...
    function _afterTokenTransfer(
        ...
        // check blacklist
        // tokens can not be transferred to the blacklisted account
        // blacklisted account can transfer tokens to the owner, or burn them
        require(
            (to == address(0) || to == owner()) ||
