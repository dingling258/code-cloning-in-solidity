...
contract CryptoEmpireTycoonPresale is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public Treferralrewards;
    ...
    constructor(address payable _paymentReceiver,IERC20 _USDT,address _priceFeedETH) {
        
        USDT=_USDT;
        paymentReceiver = _paymentReceiver;
        phases.push(Phase(50000000000 * 1E18, 125 * 1E17));           
    ...
    function BuyWithETH(address _ref) external payable {
        ...
        if(referalPercent!=0)
        {
         referETHamt=  msg.value*referalPercent/100; 
        payable(_ref).transfer(referETHamt);
        }
        }
        uint256 referUSDTamt= (referETHamt * (getLatestPriceETH())) / (1e30);
        Treferralrewards+=referUSDTamt;
        
        uint256 remaining=msg.value-referETHamt;
         

        payable(paymentReceiver).transfer(remaining);
        ...
        function BuyTokenWithUSDT(uint256 _amt,address _ref) external {
        ...
        require(isBlacklist[msg.sender]==false,"Presale : you are blacklisted");
        require(tx.origin == msg.sender,"Presale : caller is a contract");
        uint256 referUSDTamt;
        if(_ref!=address(0))
        {
        if(referalPercent!=0)
        {
         referUSDTamt=  _amt*referalPercent/100; 
        IERC20(USDT).safeTransferFrom(msg.sender,_ref,referUSDTamt);
        }
        }
        Treferralrewards+=referUSDTamt;
        uint256 remaining=_amt-referUSDTamt;
        IERC20(USDT).safeTransferFrom(msg.sender,paymentReceiver,remaining);
