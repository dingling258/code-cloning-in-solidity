...
contract RaiseAllInxPugsyPoker is IERC20 {
    ...
    uint256 constant private startingSupply = 7_770_000_000;
    string constant private _name = "Raise All-In x Pugsy Poker";
    string constant private _symbol = "$RAISE";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x3e51878B5cC7db6867949A77Aad27992f2B73592),
        operations: payable(0xE44A109cC5782c080629EA8774be069174257187),
        project: payable(0xF37428dfeE044416Bd200210bD228D1604c9f2D4)
    });
...
//================================================ BLACKLIST
    ...
    function setRatios(uint16 liquidity, uint16 operations, uint16 project, uint16 marketing) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.operations = operations;
        _ratios.project = project;
        _ratios.totalSwap = liquidity + marketing + operations + project;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable project, address payable operations) external onlyOwner {
        require(marketing != address(0) && project != address(0) && operations != address(0), "Cannot be zero address.");
        _taxWallets.marketing = payable(marketing);
        _taxWallets.project = payable(project);
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        uint256 projectBalance = (amtBalance * ratios.operations) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - (projectBalance + operationsBalance);
        if (ratios.marketing > 0) {
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: 55000}("");
        }
        if (ratios.project > 0) {
            (success,) = _taxWallets.project.call{value: projectBalance, gas: 55000}("");
        }        
        if (ratios.operations > 0) {
            (success,) = _taxWallets.operations.call{value: operationsBalance, gas: 55000}("");
        }
    }
