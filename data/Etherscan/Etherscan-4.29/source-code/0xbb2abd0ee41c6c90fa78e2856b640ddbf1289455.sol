...
/*
    Web     : https://dynamoai.co
    App     : https://app.dynamoai.co

    Twitter : https://twitter.com/DynamoAI_
    Medium  : https://dynamoai.medium.com
    Telegram: https://t.me/dynamoai_project
*/
...
contract DynamoAI is ERC20, Ownable {
    ...
    uint256 public dnaiOnePercent;

    bool public limitsInEffect = true;
    bool public dnaiTradingActive = false;
    bool public swapEnabled = false;

    uint256 private dnaiLaunchedBlock;
    uint256 private dnaiLaunchDelay = 2;
    ...
    constructor() ERC20("Dynamo AI", unicode"DNAI") {
        // uniswap router
        ...
        MarketingWallet = address(0xD5d1DFA1d9340a9E15C8c0feab5A04B6aEF0e425);
        ...
        dnaiOnePercent = 1_000_000 * 1e18; // 1% from total supply
    ...
    function enableLaunch() external onlyOwner {
        dnaiTradingActive = true;
        swapEnabled = true;
        dnaiLaunchedBlock = block.number;
    }

    // remove limits after token is stable
    function removeDnaiLimits() external onlyOwner returns (bool) {
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (block.number < dnaiLaunchedBlock + dnaiLaunchDelay) {dnaiFees = amount.mul(40).div(100);} else {
                // on sell
                if (automatedMarketMakerPairs[to] && sellTotalFees > 0) {
                    dnaiFees = amount.mul(sellTotalFees).div(100);
                    tokensForDev += (dnaiFees * sellDevFee) / sellTotalFees;
                    tokensForMarketing += (dnaiFees * sellMarketingFee) / sellTotalFees;
                    tokensForShare += (dnaiFees * sellShareFee) / sellTotalFees;
                }
                // on buy
                else if (automatedMarketMakerPairs[from] && buyTotalFees > 0) {
                    dnaiFees = amount.mul(buyTotalFees).div(100);
                    tokensForDev += (dnaiFees * buyDevFee) / buyTotalFees;
                    tokensForMarketing += (dnaiFees * buyMarketingFee) / buyTotalFees;
                    tokensForShare += (dnaiFees * buyShareFee) / buyTotalFees;
                }
            }

            if (dnaiFees > 0) {
                super._transfer(from, address(this), dnaiFees);
            }
    ...
    function swapBack(uint256 amt) private {
        ...
        if (contractBalance > dnaiOnePercent) {
        ...
        function manualDnaiSwap(uint256 amount) external {
