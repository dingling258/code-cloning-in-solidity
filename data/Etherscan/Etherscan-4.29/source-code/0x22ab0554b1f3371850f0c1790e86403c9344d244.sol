...
abstract contract Context {
    function _msgSender() internal view virtual returns (address payable) {
        ...
        // Old code was       return msg.sender;
...
contract TokenSwapper {
        ...
        error SwapNumberAlreadyUsed();
        error ExpiryDateNotWithinFutureYear();
        error AddressValueZeroOrInvalid();
        error TokensAddressesCannotBeSame();
        error TokenAmountCannotBeZero(); 
        error SwapNumberDoesNotExistOrExpired();
        error SwapNoMoreOpen();
        error InCorrectSwapExecute(); 
        error SwapNotYetExpired();
    ...
    event Opened(address indexed originator, address executor, address indexed openingToken, uint256 tokensToOpen, address closingToken, uint256 tokensToClose);
    event Closed(address indexed originator, address executor, address indexed openingToken, uint256 tokensToOpen, address closingToken, uint256 tokensToClose);
    event Expired(address indexed originator, address executor, address indexed openingToken, uint256 tokensToOpen, address closingToken, uint256 swapNumber);
    ...
    function open(uint256 swapNumber, address _executor, address _openingToken, uint256 _tokensToOpen, address _closingToken, uint256 _tokensToClose, uint256 _expiry) external returns(bool) {
        ...
        if( allSwaps[msg.sender][swapNumber].expiry > 0 ) {
            revert SwapNumberAlreadyUsed();      
        }

        if( _expiry <= block.timestamp || _expiry > block.timestamp + 365 days ) {
        ...
        //emit event
        emit Opened(msg.sender, _executor, _openingToken, _tokensToOpen, _closingToken, _tokensToClose);
    ...
    function close(address originator, uint256 swapNumber) external returns(bool) {
        ...
        if( !(allSwaps[originator][swapNumber].expiry > 0) ) {
        ...
        //emit event
        emit Closed(originator, swap.executor , swap.openingToken, swap.tokensToOpen, swap.closingToken, swap.tokensToClose);
    ...
    function expire(address originator, uint256 swapNumber) external returns(bool) {
        ...
        //emit event
        emit Expired(originator, swap.executor , swap.openingToken, swap.tokensToOpen, swap.closingToken, swap.tokensToClose);
