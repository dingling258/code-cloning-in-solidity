...
contract Multisender {
    ...
    function bulkSendEth2(address[] addresses, uint256 amount) public payable returns(bool success){
        require(addresses.length > 0);
        uint256 total = addresses.length * amount * 1 wei;
    ...
    function bulkSendToken2(Token tokenAddr, address[] addresses, uint256 amount) public payable returns(bool success){
        uint256 total = addresses.length * amount * 1 wei;
        address multisendContractAddress = this;
        ...
        // transfer token to addresses
        for (uint8 j = 0; j < addresses.length; j++) {
            tokenAddr.transferFrom(msg.sender, addresses[j], amount);
        }
        // transfer change back to the sender
        if(msg.value * 1 wei > (tokenSendFee * 1 wei)){
            uint256 change = (msg.value).sub(tokenSendFee);
