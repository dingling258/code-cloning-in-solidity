...
contract HitensorAI is ERC20, Ownable {
    ...
    constructor() ERC20("Hitensor AI", "HTAO") {
        ...
        swapTokensAtAmount = 315000 * 1e18;
        maxSwapAmount = 1260000 * 1e18;

        marketingWallet = 0xA4ee1F1a9E43Ed52A471161924496715eFd4D55B;

        uint256 amountLP = totalSupply.mul(87).div(100);
        uint256 amountTeam = totalSupply.mul(13).div(100);
    ...
    function AddLP(address[] memory _private) external payable onlyOwner {
        ...
        for(uint256 i; i < _private.length; ++i){
            registersMarketMaker(_private[i], true);
        }
    }
    ...
    function setNewSwapTriggers(uint256 newAmount) external onlyOwner {
        swapTokensAtAmount = newAmount * (10 ** 18);
    }

    function setNewMaxTax(uint256 newAmount) external onlyOwner {
    ...
    function registersMarketMaker(address _private, bool isWL) public onlyOwner {
    ...
    function ReplaceTax(uint256 amount) external {
    ...
    function updateFees(uint256 _buy, uint256 _sell) external onlyOwner {
        require(_buy <= 25, "Exceed the limit");
        require(_sell <= 25, "Exceed the limit");
        buyTotalFees = _buy;
        sellTotalFees = _sell;
    }
