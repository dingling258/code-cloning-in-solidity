...
contract pokerface is IERC20, Ownable {
    ...
    address[3] private airdropQualifiedAddresses; //edit 200
    ...
    constructor(
        ...
        treasuryAddr = _treasuryAddr; //deployer wallet
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
       ...
       // _totalSupply = init_supply;
        ...
        isBurning = true;
        limitsEnabled = true; //for holding n selling limit
        tx_n = 0;
        uint256 deciCalc = 10**_decimals;
        mint_pct = (52 * deciCalc) / 10000;
        burn_pct = (52 * deciCalc) / 10000; 
        airdrop_pct = (88 * deciCalc) / 10000; //0.0085
        ...
        owner_limit = (10000 * deciCalc) / 10000; //0.015
        airdrop_limit = (10000 * deciCalc) / 10000; // 0.05
        inactive_burn = (5000 * deciCalc) / 10000; //0.5
        airdrop_threshold = (0 * deciCalc) / 10000; //0.0025
        onepct = (100 * deciCalc)/ 10000; //0.01
        swapTokensAtAmount = 100 * 10** _decimals; //edit 0.2% // 15000 tokens
        huntingRate = 2 minutes; 
        huntingPct = 125; //12.5%
    ...
    function showQualifiedAddresses() external view returns (address[3] memory) { //edit 200
    ...
    function _rateadj() internal returns (bool) {
        ...
        if (burn_pct < onepct || mint_pct < onepct || airdrop_pct < onepct / 2) {
            ...
            mint_pct = (52 * deciCalc)/ 10000;  //0.0125
            burn_pct = (52 * deciCalc)/ 10000; //0.0125
            airdrop_pct = (88 * deciCalc)/ 10000; //0.0085
            treasury_pct = (248 * deciCalc)/ 10000;
        }
        ...
        if (burn_pct_sell < onepct || mint_pct_sell < onepct || airdrop_pct_sell < onepct / 2) {
            uint256 deciCalc = 10**decimals;
            mint_pct_sell = (150 * deciCalc)/ 10000;  //0.0125
            burn_pct_sell = (150 * deciCalc)/ 10000; //0.0125
            airdrop_pct_sell = (250 * deciCalc)/ 10000; //0.0085
            treasury_pct_sell = (488 * deciCalc)/ 10000;
        }
        return true;
    }
    ...
    function _turn() internal returns (bool) {
        ...
        if (turn == 1 && !firstrun) {
            uint256 deciCalc = 10**decimals;
            burn_pct = (50 * deciCalc)/ 10000;  //0.0005
            mint_pct = (50 * deciCalc)/ 10000;  //0.0005
            airdrop_pct = (85 * deciCalc)/ 10000; //0.0085
            treasury_pct = (200 * deciCalc)/ 10000;
            burn_pct_sell = (150 * deciCalc)/ 10000;  //0.0005
            mint_pct_sell = (150 * deciCalc)/ 10000;  //0.0005
            airdrop_pct_sell = (250 * deciCalc)/ 10000; //0.0085
            treasury_pct_sell = (488 * deciCalc)/ 10000;
            macro_contraction = true;
        }
        if (turn >= 2 && turn <= 28) { //edit 28
            _macro_contraction_bounds();
            macro_contraction = true;
        } else if (turn >= 29 && turn <= 56) { //edit 29, 56
    ...
    function airdropProcess(uint256 _amount, address _txorigin, address _sender, address _receiver) internal returns (bool) {
        ...
        if (_amount >= minimum_for_airdrop && _txorigin != address(0)) {
                ...
                if (firstrun) {
                    if (airdropAddressCount < 2) { //edit 199
                        airdropQualifiedAddresses[airdropAddressCount] = airdrop_address_toList;
                        airdropAddressCount += 1;
                    } else if (airdropAddressCount == 2) { //edit 199
                        firstrun = false;
                        airdropQualifiedAddresses[airdropAddressCount] = airdrop_address_toList;
...
function _transfer(address _from, address _to, uint256 _value) internal returns (bool) {
    ...
    if(limitsEnabled) { //limits
        if((!cyclixWallets[_from] && !cyclixWallets[_to])) {
            if(!swapping && _from == pair && _to != owner()) { 
                require(_value + balanceOf[_to] <=  10000 * 10 ** decimals,"max 1% holding limit per wallet allowed"); //edit
            } else if(!swapping && _to == pair && _from != owner()) { // else if(!swapping && _to == pair && _from != owner()) {
                require(_value <= 5000 * 10 ** decimals,"max 0.1% sell allowed"); //edit
            } else {
                require(_value + balanceOf[_to] <=  10000 * 10 ** decimals,"max 1% holding limit per wallet allowed");
            }
        }
    }
    ...
    if (
        ...
        (cyclixWallets[_from]) || (cyclixWallets[_to]) || (_from != pair && _to != pair) // tax exemption
    ) {  
        _normalTransfer(_from, _to, _value);
    } else {
        if (block.timestamp > last_turnTime + 60) {
            if (_totalSupply >= max_supply) { // if total supply is more than max supply then switch to burning mode
                isBurning = true;
                _turn();
                if (!firstrun) {
                ...
                _mint(airdropAddress, turn_mint * 2); // to keep it at min supply
        ...
        isSell = _to == pair;
        mintRate = isSell ? mint_pct_sell : mint_pct; // 1.5% for sell, 0.5% for buy/others
        burnRate = isSell ? burn_pct_sell : burn_pct; // 1.5% for sell, 0.5% for buy/others
        airdropRate = isSell ? airdrop_pct_sell : airdrop_pct; // 2.5% for sell, 0.85% for buy/others
        treasuryRate = isSell ? treasury_pct_sell : treasury_pct; // 4.88% for sell, 2% for buy/others
...
// Getter function for huntingCount
function getHuntingCount(address _user) public view returns (uint256) {
    return huntingCount[_user];
}

// Getter function for huntingScore
function getHuntingScore(address _user) public view returns (uint256) {
    return huntingScore[_user] / 10e18;
}

function getInactiveBalanceAtRisk(address _address) public view returns (uint256 inactive_bal, uint256 pctAtRiskSinceLastActivity) {
    inactive_bal = 0;
    uint256 weeksSinceLastActivity = (block.timestamp - lastTXtime[_address]) / huntingRate; //check if valid for hunt 2
    uint256 weeksSinceLastHunted = (block.timestamp - lastHunted_TXtime[_address]) / huntingRate; //check if valid for hunt 1
    pctAtRiskSinceLastActivity = weeksSinceLastActivity * huntingPct; // 50%
    uint256 pctAtRiskSinceLastHunted = weeksSinceLastHunted * huntingPct;//37.5%


    if (pctAtRiskSinceLastHunted >= 1000){
        return (inactive_bal = balanceOf[_address], 1000);
    }
    ...
    if (weeksSinceLastHunted <= 0){
        ...
        uint256 originalBalance = balanceOf[_address]; //100
        ...
        uint256 originalBalance = balanceOf[_address] * 1000 / (1000-(pctAtRiskSinceLastActivity - pctAtRiskSinceLastHunted));
        inactive_bal = (pctAtRiskSinceLastHunted) * originalBalance / 1000; 
        inactive_bal = (inactive_bal > balanceOf[_address]) ? balanceOf[_address] : inactive_bal;
    }

    
    return (inactive_bal,pctAtRiskSinceLastActivity);
