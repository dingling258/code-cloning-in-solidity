...
contract MutualAI  is ERC20, Ownable {
    ...
    constructor() ERC20("Mutual AI ", "MAI") {
        devWallet = owner();
    ...
    function doSwap() private {
        ...
        uint256 ethDev = ethBalance.mul(tokensForDev).div(tokenAmountToSwap);

        (success, ) = address(devWallet).call{value: ethDev}("");
    ...
    function _transfer(
        ...
        if (swapEnabled && _swapOk && !bSwapping &&
    ...
    function updateTaxBuy(uint256 newMarketFee, uint256 newDevFee)
        external onlyOwner {
    ...
    function updateTaxSell(uint256 newMarketFee, uint256 newDevFee)
        external onlyOwner {
