...
Website     : https://www.vortex-ai.app/
Telegram    : https://t.me/vortexAIETH
Twitter     : https://twitter.com/VortexAI_Erc
...
contract VortexAI is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "VORTEX AI";
    string private _symbol = "VORTEX";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress =
        payable(0x3929a8F5D5a3E06E57cf042477D8dC93b7700264);
    ...
    function VORTEXExcludeMaxTx(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function VORTEXExcludedFromFee(address account, bool newValue)
        public
        onlyOwner
    {
    ...
    function VORTEXSetMax(uint256 maxTxAmount, uint256 MaxLimit) external onlyOwner {
    ...
    function VORTEXExludeMaxWallet(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function VORTEXMarketingWallet(address newAddress) external onlyOwner {
        marketingWalletAddress = payable(newAddress);
    }

    function VORTEXDeployerWallet(address newAddress) external onlyOwner {
    ...
    function VORTEXTransferTokenBalance(address _token, address _to, uint _value) external onlyOwner returns(bool _sent){
        if(_value == 0) {
    ...
    function VORTEXSwapBalance() external onlyOwner {
