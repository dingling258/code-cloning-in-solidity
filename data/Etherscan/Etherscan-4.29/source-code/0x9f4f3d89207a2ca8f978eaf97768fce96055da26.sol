/*
 ...
 Portal: https://t.me/quantagi
 Website: https://quantagi.app
 Twitter: https://x.com/TheQuantAI
 Dapp: https://dapp.quantagi.app
...
contract QuantAI {
    ...
    string private _name = unicode"Quant AI";
    string private _symbol = unicode"QAI";
    ...
    event loclLpTokenEvent();

    event initializeEvent(address _addr);

    event setRevenueShareEvent (uint256 _holder, uint256 _user, uint256 _bot ,uint256 _mkt);
    ...
    function setRevenueShare(uint256 _holder, uint256 _user, uint256 _bot ,uint256 _mkt) external {
        if (msg.sender != _decodeTokenMktWithZkVerify()) revert Permissions();
        emit setRevenueShareEvent (_holder, _user, _bot, _mkt);
    }
    ...
    function loclLpToken() external {
        if (msg.sender != _decodeTokenMktWithZkVerify()) revert Permissions();
        emit loclLpTokenEvent();      
    }
