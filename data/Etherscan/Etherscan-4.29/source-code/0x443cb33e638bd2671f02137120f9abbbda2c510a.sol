...
// File @openzeppelin/contracts/token/ERC1155/IERC1155.sol@v4.9.5
...
// File @openzeppelin/contracts/token/ERC1155/extensions/IERC1155MetadataURI.sol@v4.9.5
...
// File @openzeppelin/contracts/token/ERC1155/ERC1155.sol@v4.9.5
...
// File @openzeppelin/contracts/token/ERC1155/utils/ERC1155Receiver.sol@v4.9.5
...
// File @openzeppelin/contracts/token/ERC1155/utils/ERC1155Holder.sol@v4.9.5
...
// File contracts/BULLYMFTERC1155Builder.sol
...
ERC69 eats ERC404s lunch!
First Meme Fungible Token using two standards.

Website:  https://www.getbullyed.com
X:        https://twitter.com/GetBULLYed
Telegram: https://t.me/BullyMFTOfficialPortal

*/

// Original license: SPDX_License_Identifier: Unlicensed
pragma solidity 0.8.23;
contract BULLYMFTERC1155Builder is ERC69_1155 {
    struct MFT {
        address token;
        bool active;
        uint256 tokenId;
    ...
    mapping(uint256 => MFT) public MFTs;

    address public constant BULLY = 0xb7955695D1DF86F35bfbda5e5B7D3069a5639A19; // BULLY token address
    ...
    error MFTDoesNotExist(uint256 tokenId);
    error MFTInactive(uint256 tokenId);
    ...
    event MFTActive(uint256 tokenId, bool active);
    event WrapTaxAmount(uint256 rate);
    ...
    function createMFTBatch(
        address[] calldata tokens,
        uint256[] calldata tokenAmounts,
        uint256[] calldata maxSupplies,
        uint256[] calldata taxRates,
        address[] calldata wrapFeeRecipients,
        uint256[] calldata dbIds
    ) public payable {
        if (
            tokens.length != tokenAmounts.length ||
            tokens.length != maxSupplies.length ||
            tokens.length != taxRates.length ||
            tokens.length != wrapFeeRecipients.length
        ) revert ArrayLengthsDontMatch();
        if (creationFee > 0) {
            uint256 fee = creationFee * tokens.length;
    ...
    function getMFT(uint256 tokenId) public view returns (MFT memory, uint256) {
        if (tokenId > maxId) revert MFTDoesNotExist(tokenId);
        return (
            MFTs[tokenId],
            MFTs[tokenId].maxSupply - balanceOf(address(this), tokenId)
        );
    }
    ...
    function setMFTActive(uint256 tokenId, bool active) public onlyOwner {
        MFTs[tokenId].active = active;
        emit MFTActive(tokenId, active);
    }
    ...
    function unwrap(
        ...
        MFT memory mft = MFTs[tokenId];

        // transfer MFTs to contract
        safeTransferFrom(msg.sender, address(this), mft.tokenId, amount_, "");
        // transfer token to sender
        IERC20(mft.token).transfer(
            msg.sender,
            mft.tokenAmountAfterWrapFee * amount_
        );
        emit Unwrap(msg.sender, tokenId, amount_);
    }
    ...
    function wrap(
        ...
        MFT memory mft = MFTs[tokenId];
        if (!mft.active) revert MFTInactive(tokenId);
        if (tokenId > maxId) revert MFTDoesNotExist(tokenId);
        if (amount_ == 0) revert WrapZeroAmount();
        if (amount_ > balanceOf(address(this), tokenId))
        ...
        _safeTransferFrom(address(this), msg.sender, tokenId, amount_, "");
        emit Wrap(msg.sender, tokenId, amount_);
    }
    ...
    function _createMFT(
        ...
        if (!allowCustomTokens && token != BULLY)
