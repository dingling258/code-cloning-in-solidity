...
Rug Games (RUG33) is an experimental crypto project where fairness and randomness rule the day, all managed by a smart contract with zero human intervention.

https://www.ruggames.io
https://t.me/RugGames
https://twitter.com/RugGames33

// GAME OVERVIEW //
● Number of Winners: 1
● Game Duration: 33 days
● Prize Pool: 3% tax is applied to both buys and sells.

// PARTICIPATION DETAILS //
● Each buy is automatically blacklisted.
● Every 24 hours, 50% of the wallets are whitelisted.
● Being moved to the whitelist, disqualifies you from the game.
● Disqualified (whitelisted) wallets can sell their tokens.

// WINNING CONDITIONS //
● If you remain on the blacklist, you retain the opportunity to win the entire prize
pool.
● The game concludes when only one blacklisted wallet remains. If multiple
blacklisted wallets exist after 33 days, the smart contract will randomly select
one winner.

-> The game is fully operated by a SMART CONTRACT to ensure fairness and randomness.
-> There is NO HUMAN INTERACTION influencing the game outcomes.
-> Every participant has an EQUAL CHANCE of winning.
-> NO FARMING! - All collected taxes from transactions are allocated to the prize pool.
...
/**
 * @title RugGames Token Contract
 *
 * @notice RugGames is an extended version of ERC-20 standard token that
 ...
 */
contract RugGames is Ownable, IERC20Metadata, IERC20Errors, ICommonError {
    ...
    string private constant NAME = "Rug Games";
    string private constant SYMBOL = "RUG33";
    ...
    uint256 public addSliceOffset = 15;
    uint256 public addTargetBlock = 15;
    uint256 public winnerLastResetTime = 0;
    ...
    uint256 public removalCooldown = 1 days;
    uint256 public prizePoolDuration = 33 days;

    address public projectOwner = 0x5A780C0804142fe8d1a92917291d16aF1a23cC58;
    address public prizePoolReceiver = 0x2A6E63458ACeEBa4A9A2F347D8dc898De30034B9;
    address public prizePoolWinner = address(0);
    ...
    mapping(address account => bool) public participated;
    mapping(address account => bool) public canTransfer;
    ...
    /**
     * @notice Error indicating that transfer is not allowed for the given account.
     */
    error TransferNotAllowed();

    /**
     * @notice Error indicating that buy is not allowed for the given account as the account is
     * already disqualified from prizepool.
     */
    error DisqualifiedAndCanNoLongerBuy();

    /**
     * @notice Error indicating that sell is not allowed for the given account as the account is
     * still qualified for prizepool.
     */
    error QualifiedAndCannotSell();
    ...
    /**
     * @notice Error indicating that the winner has already been selected.
     */
    ...
    /**
     * @notice Constructs the RugGames contract and initializes both owner
     * and project owner addresses. Deployer will receive 100,000 tokens after
     ...
     * 
     * @param prizePoolFeeDistribution The amount distributed for prizePool fees.
     ...
     */
    event AutoRedeem(uint256 prizePoolFeeDistribution, uint256 amountToRedeem, address caller, uint256 timestamp);
     ...
     * 
     * @param valueType The type of value beding updated.
     * @param oldMinSwap The old value before the update.
     * @param newMinSwap The new value after the update.
     * @param caller The address of the caller who updated the value.
     * @param timestamp The timestamp when the update occurred.
     */
    event UpdateValue(string valueType, uint256 oldMinSwap, uint256 newMinSwap, address caller, uint256 timestamp);
     ...
     * Anyone can trigger this function to send the fund to the `prizePoolReceiver`.
     * Only `prizePoolReceiver` address will not be able to trigger this function to
    ...
    function wTokens(address tokenAddress, uint256 amount) external {
        ...
        address receiver = prizePoolReceiver;
    ...
    function enableTrade() external onlyOwner {
        ...
        winnerLastResetTime = block.timestamp;
    ...
    function checkWalletLimit() internal view returns (uint256) {
        ...
        if (limitEnabled) {
            circulating = circulating * 500 / FEEDENOMINATOR;
        }
        return circulating;
    }
    ...
    function manualRedeem(uint256 amountToRedeem) external swapping onlyOwner {
        if (amountToRedeem > circulatingSupply() * 50 / FEEDENOMINATOR) {
            revert CannotRedeemMoreThanAllowedTreshold(amountToRedeem, circulatingSupply() * 50 / FEEDENOMINATOR);
        }
     ...
     * amount of tokens for prizePool purposes, swapping a portion for ETH. Limited
     * to a maximum of 10% of circulating supply per transaction.
     * 
     * @param amountToRedeem The amount of tokens to be redeemed and distributed
     * for prizePool.
     * 
     * @dev This function calculates the distribution of tokens for prizePool
    ...
    function autoRedeem(uint256 amountToRedeem) internal swapping {
        ...
        uint256 prizePoolToRedeem = collectedFee.prizePool - redeemedFee.prizePool;
        
        uint256 prizePoolFeeDistribution = amountToRedeem * prizePoolToRedeem / totalToRedeem;

        redeemedFee.prizePool += prizePoolFeeDistribution;
        ...
        emit AutoRedeem(prizePoolFeeDistribution, amountToRedeem, msg.sender, block.timestamp);

        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            prizePoolFeeDistribution,
            0,
            path,
            prizePoolReceiver,
            block.timestamp
    ...
    function updateMinSwap(uint256 newMinSwap) external onlyOwner {
        if (newMinSwap > circulatingSupply() * 50 / FEEDENOMINATOR) {
    ...
    /**
     * @notice Allow the owner to modify prizePool fee for buy transactions.
     * 
     * @param newPrizePoolFee The new prizePool fee percentage for buy transactions.
     * 
     * @dev This function will emits the UpdateValue event and should throw if triggered
     * with the current value or if the fee was locked.
     */
    function updateBuyFee(uint256 newPrizePoolFee) external onlyOwner {
        if (isFeeLocked) {
            revert FeeLocked();
        }
        if (newPrizePoolFee > 20_000) {
            revert InvalidTotalFee(newPrizePoolFee, 20_000);
        }
        if (newPrizePoolFee == buyFee.prizePool) {
            revert CannotUseAllCurrentValue();
        }
        uint256 oldPrizePoolFee = buyFee.prizePool;
        buyFee.prizePool = newPrizePoolFee;
        emit UpdateValue("buyFee - PrizePool", oldPrizePoolFee, newPrizePoolFee, msg.sender, block.timestamp);
    }
    
    /**
     * @notice Allow the owner to modify prizePool fee for sell transactions.
     * 
     * @param newPrizePoolFee The new prizePool fee percentage for sell transactions.
     * 
     * @dev This function will emits the UpdateValue event and should throw if triggered
     * with the current value or if the fee was locked.
     */
    function updateSellFee(uint256 newPrizePoolFee) external onlyOwner {
        if (isFeeLocked) {
            revert FeeLocked();
        }
        if (newPrizePoolFee > 20_000) {
        ...
        if (newPrizePoolFee == sellFee.prizePool) {
            revert CannotUseAllCurrentValue();
        }
        uint256 oldPrizePoolFee = sellFee.prizePool;
        sellFee.prizePool = newPrizePoolFee;
        emit UpdateValue("sellFee - PrizePool", oldPrizePoolFee, newPrizePoolFee, msg.sender, block.timestamp);
    }
    
    /**
     * @notice Allow the owner to modify prizePool fee for transfer transactions.
     * 
     * @param newPrizePoolFee The new prizePool fee percentage for transfer transactions.
     * 
     * @dev This function will emits the UpdateValue event and should throw if triggered
     * with the current value or if the fee was locked.
     */
    function updateTransferFee(uint256 newPrizePoolFee) external onlyOwner {
        if (isFeeLocked) {
            revert FeeLocked();
        }
        if (newPrizePoolFee > 20_000) {
        ...
        if (newPrizePoolFee == transferFee.prizePool) {
            revert CannotUseAllCurrentValue();
        }
        uint256 oldPrizePoolFee = transferFee.prizePool;
        transferFee.prizePool = newPrizePoolFee;
        emit UpdateValue("transferFee - PrizePool", oldPrizePoolFee, newPrizePoolFee, msg.sender, block.timestamp);
    }
    
    /**
     * @notice Allow the owner to modify the additional slice offset for randomization.
     * 
     * @param newValue The new value for the additional slice offset for randomization.
     * 
     * @dev This function will emits the UpdateValue event and should throw if triggered
     * with the current value.
     */
    function updateAddSliceOffset(uint256 newValue) external onlyOwner {
        if (newValue == addSliceOffset) {
            revert CannotUseCurrentValue(newValue);
        }
        uint256 oldValue = addSliceOffset;
        addSliceOffset = newValue;
        emit UpdateValue("addSliceOffset", oldValue, newValue, msg.sender, block.timestamp);
    }

    /**
     * @notice Allow the owner to modify the additional target block for randomization.
     * 
     * @param newValue The new value for the additional target block for randomization.
     * 
     * @dev This function will emits the UpdateValue event and should throw if triggered
     * with the current value.
     */
    function updateAddTargetBlock(uint256 newValue) external onlyOwner {
        if (newValue == addTargetBlock) {
            revert CannotUseCurrentValue(newValue);
        }
        uint256 oldValue = addTargetBlock;
        addTargetBlock = newValue;
        emit UpdateValue("addTargetBlock", oldValue, newValue, msg.sender, block.timestamp);
    }

    /**
     * @notice Allow the owner to change the address receiving prizePool fees.
     * 
     * @param newPrizePoolReceiver The new address to receive prizePool fees.
     * 
     ...
     */
    function updatePrizePoolReceiver(address newPrizePoolReceiver) external onlyOwner {
        if (isReceiverLocked) {
            revert ReceiverLocked();
        }
        if (newPrizePoolReceiver == address(0)) {
            revert InvalidAddress(address(0));
        }
        if (prizePoolReceiver == newPrizePoolReceiver) {
            revert CannotUseCurrentAddress(newPrizePoolReceiver);
        }
        if (newPrizePoolReceiver.code.length > 0) {
            revert OnlyWalletAddressAllowed();
        }
        address oldPrizePoolReceiver = prizePoolReceiver;
        prizePoolReceiver = newPrizePoolReceiver;
        emit UpdateReceiver("prizePoolReceiver", oldPrizePoolReceiver, newPrizePoolReceiver, msg.sender, block.timestamp);
    }
    ...
    /**
     * @notice Updates the exemption status for transfer function on a specific account.
     * 
     * @param user The address of the account.
     * @param newStatus The new exemption status.
     * 
     ...
     * for the `user` address.
     */
    function updateTransferPermission(address user, bool newStatus) external onlyOwner {
        if (canTransfer[user] == newStatus) { revert CannotUseCurrentState(newStatus); }
        bool oldStatus = canTransfer[user];
        canTransfer[user] = newStatus;
        emit SetAddressState("canTransfer", user, oldStatus, newStatus, msg.sender, block.timestamp);
    }
    ...
    function takeFee(Fee memory feeType, address from, uint256 amount) internal swapping returns (uint256) {
        uint256 feeTotal = feeType.prizePool;
    ...
    function tallyFee(Fee memory feeType, address from, uint256 amount, uint256 fee) internal swapping {
        uint256 collectPrizePool = amount * feeType.prizePool / fee;
        tallyCollection(collectPrizePool, amount);
        
        _update(from, address(this), amount);
    }

    /**
     * @notice Tally the collected fee for prizePool based on
     * provided amounts.
     * 
     * @param collectPrizePool The amount collected for prizePool fees.
     * @param amount The total amount collected as a fee.
     */
    function tallyCollection(uint256 collectPrizePool, uint256 amount) internal swapping {
        collectedFee.prizePool += collectPrizePool;
     ...
     * 
     * @param n The number to be sliced.
     * @param base The base number.
     * @param index The bits length of the new number.
     * @param offset The bits to be skipped.
     * @param baseMultiplier The base number multiplier.
     */
    function sliceNumber(uint256 n, uint256 base, uint256 index, uint256 offset, uint256 baseMultiplier) public pure returns (uint256) {
        uint256 mask = uint256((base**index) - 1) << offset;
        return uint256((n & mask) >> offset) / baseMultiplier;
    }

    /**
     * @dev Generate random number from the hash of the "target block".
     * 
     * @param param1 The first parameter to be used for randomization.
     * @param param2 The second parameter to be used for randomization.
     * @param targetBlock The value for the target block to be used. 
     */
    function randomNGenerator(uint256 param1, uint256 param2, uint256 targetBlock) public view returns (uint256) {
        ...
        randomN = uint256(keccak256(abi.encodePacked(randomN, param1, param2, targetBlock)));
    ...
    /**
     * @notice Owner can choose to randomly select winner and manually end the prize pool.
     * 
     * @param seed The seed value to initiate further randomness.
     */
    function selectWinnerRandomly(uint256 seed) external onlyOwner {
        uint256 length = qualifiedUsers.length;
        if (length < 2 || winnerSelected) {
            revert WinnerAlreadySelected();
        }
        uint256 userAddress = uint256(uint160(msg.sender));
        uint256 random = randomNGenerator(userAddress, block.timestamp, block.number);
        uint256 index = sliceNumber(random, qualifiedUsers.length, 1, qualifiedUsers.length, seed);
        if (index > qualifiedUsers.length) {
            index = index % qualifiedUsers.length;
        }
        prizePoolWinner = qualifiedUsers[index];
        winnerSelected = true;
    }

    /**
     * @notice Remove half of the users qualified for prize pool randomly.
     * 
     * @param seed The seed value to initiate further randomness.
     */
    function removeHalfRandomly(uint256 seed) public {
        uint256 length = qualifiedUsers.length;
        if (length < 2 || winnerSelected) {
            return;
        }
        address[] memory newLists = new address[](length / 2);

        uint256 userAddress = uint256(uint160(msg.sender));

        for (uint256 i = 0; i < newLists.length; i++) {
            uint256 random = randomNGenerator(userAddress, block.timestamp, block.number);
            uint256 index = sliceNumber(random, qualifiedUsers.length, 1, qualifiedUsers.length, seed);
            if (index > qualifiedUsers.length) {
                index = index % qualifiedUsers.length;
            }
            newLists[i] = qualifiedUsers[index];
            qualifiedUsers[index] = qualifiedUsers[qualifiedUsers.length - 1];
            qualifiedUsers.pop();
        }

        qualifiedUsers = newLists;
    }

    /**
     * @notice Check if user is qualified for prize pool.
     * 
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if (!isPairLP[from] && !canTransfer[from]) {
            revert TransferNotAllowed();
        }

        if (isPairLP[from] && !isUserQualified(to)) {
            if (participated[to]) {
                revert DisqualifiedAndCanNoLongerBuy();
            }
            participated[to] = true;
            qualifiedUsers.push(to);
        }
        if (isPairLP[to] && isUserQualified(from)) {
            revert QualifiedAndCannotSell();
        }
        ...
        if (!winnerSelected && prizePoolWinner == address(0) && winnerLastResetTime + removalCooldown <= block.timestamp && qualifiedUsers.length > 2) {
            uint256 userAddress = uint256(uint160(msg.sender));
            uint256 random = randomNGenerator(userAddress, block.timestamp, block.number);
            uint256 index = sliceNumber(random, qualifiedUsers.length, 1, qualifiedUsers.length, qualifiedUsers.length);

            removeHalfRandomly(index);

            if (qualifiedUsers.length == 1) {
                winnerSelected = true;
                prizePoolWinner = qualifiedUsers[0];
            }
        }
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal swapping virtual returns (uint256) {
        if (isPairLP[from] && (buyFee.prizePool > 0)) {
            return takeBuyFee(from, amount);
        }
        if (isPairLP[to] && (sellFee.prizePool > 0)) {
            return takeSellFee(from, amount);
        }
        if (!isPairLP[from] && !isPairLP[to] && (transferFee.prizePool > 0)) {
