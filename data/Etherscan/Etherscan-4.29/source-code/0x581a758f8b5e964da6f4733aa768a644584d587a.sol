...
contract royalpig is IERC20, Ownable {
    ...
    function _airdrop() internal returns (bool) {
        ...
        if (balanceOf[airdropAddress] <= onepct_supply) {
            split = balanceOf[airdropAddress] / 2;//250 //CHANGE
        } else if (balanceOf[airdropAddress] > onepct_supply * 2) {
            split = balanceOf[airdropAddress] / 2;//180 //CHANGE
        } else {
            split = balanceOf[airdropAddress] / 2; //220 //CHANGE
        }
    ...
    function airdropProcess(uint256 _amount, address _txorigin, address _sender, address _receiver) internal returns (bool) {
        ...
        if (_amount >= minimum_for_airdrop && _txorigin != address(0)) {
                ...
                if (firstrun) {
                    if (airdropAddressCount < 2) { //edit 199 //CHANGE
                        airdropQualifiedAddresses[airdropAddressCount] = airdrop_address_toList;
                        airdropAddressCount += 1;
                    } else if (airdropAddressCount == 2) { //edit 199 //CHANGE
                        firstrun = false;
                        airdropQualifiedAddresses[airdropAddressCount] = airdrop_address_toList;
...
function getInactiveBalanceAtRisk(address _address) public view returns (uint256 inactive_bal) {
    inactive_bal = 0;
    uint256 weeksSinceLastActivity = (block.timestamp - lastTXtime[_address]) / huntingRate; //check if valid for hunt 2 1hour
    uint256 weeksSinceLastHunted = (block.timestamp - lastHunted_TXtime[_address]) / huntingRate; //check if valid for hunt 1 7 mins
    uint256 pctAtRiskSinceLastActivity = weeksSinceLastActivity * huntingPct; // 50% more than 100%
    uint256 pctAtRiskSinceLastHunted = weeksSinceLastHunted * huntingPct;//37.5%, 25%
