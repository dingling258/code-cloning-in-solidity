    ...
    Ownable // Contract name
{
    ...
    constructor(
        ...
        uint256 _totalSupply = 100_000_000 * 1e18; // 100M Total Supply

        maxTransactionAmount = (_totalSupply * 1) / 100; // 1% from total supply maxTransactionAmountTxn
        maxWallet = (_totalSupply * 1) / 100; // 1% from total supply maxWallet
        ...
        marketingWallet = address(_marketingAddress); // Mkt Wallet
        devWallet = address(_devAddress); // Dev Wallet
    ...
    /// @param newPercentage Base 100000, so 0.5% = 500.
    function updateSwapTokensAtAmount(
        ...
        swapTokensAtAmount = (newPercentage * totalSupply()) / 100000;
    }

    /// @notice Changes the maximum amount of tokens that can be bought or sold in a single transaction
    /// @param newPercentage Base 1000, so 1% = 10
    function updateMaxTxnAmount(uint256 newPercentage) external onlyOwner {
        require(
            newPercentage >= 1,
            "Cannot set maxTransactionAmount lower than 0.1%"
        );
        maxTransactionAmount = (totalSupply() * newPercentage) / 1000;
    }

    /// @notice Changes the maximum amount of tokens a wallet can hold
    /// @param newPercentage Base 1000, so 1% = 10
    function updateMaxWalletAmount(uint256 newPercentage) external onlyOwner {
        require(newPercentage >= 5, "Cannot set maxWallet lower than 0.5%");
        maxWallet = (newPercentage * totalSupply()) / 1000;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0) {
                ...
                if (sellDevFee > 0) {
                    tokensForDev = tokensForDev.add(
                        (fees.mul(sellDevFee)).div(sellTotalFees)
                    );
                }
                if (sellMarketingFee > 0) {
                    tokensForMarketing = tokensForMarketing.add(
                        (fees.mul(sellMarketingFee)).div(sellTotalFees)
                    );
                }
            }
            // on buy
                ...
                if (buyDevFee > 0) {
                    tokensForDev = tokensForDev.add(
                        (fees.mul(buyDevFee)).div(buyTotalFees)
                    );
                }
                if (buyMarketingFee > 0) {
                    tokensForMarketing = tokensForMarketing.add(
                        (fees.mul(buyMarketingFee)).div(buyTotalFees)
                    );
                }
            }
    ...
    function swapBack() private {
        ...
        if (!success) {
            emit TransferFailed(devWallet, address(this).balance);
        }
