...
interface IxVexaris is IERC20 {
    function rebase( uint256 VexarisProfit_, uint epoch_) external returns (uint256);
...
contract VexarisStaking is Ownable {
    ...
    IERC20 public immutable Vexaris;
    IxVexaris public immutable xVexaris;
    ...
    constructor ( 
        ...
        require( _Vexaris != address(0) );
        Vexaris = IERC20(_Vexaris);
        require( _xVexaris != address(0) );
        xVexaris = IxVexaris(_xVexaris);
    ...
    /**
        @notice stake Vexaris to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient, LOCKUPS _lockup ) external returns ( bool ) {
        ...
        Vexaris.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( xVexaris.gonsForBalance( _amount ) ),
        ...
        if( _lockup == LOCKUPS.MONTH1 ) { // 1 - 1.25 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IxVexaris( xVexaris ).gonsForBalance( _amount.mul(25).div(100) ),
                gonsAccount: IxVexaris( xVexaris ).gonsForBalance( _amount ),
        ...
        if( _lockup == LOCKUPS.MONTH3 ) { // 2 - 1.5 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IxVexaris( xVexaris ).gonsForBalance( _amount.mul(50).div(100) ),
                gonsAccount: IxVexaris( xVexaris ).gonsForBalance( _amount ),
                initialDeposit: _amount,
                multiplier: 150,
        ...
        if( _lockup == LOCKUPS.MONTH6 ) { // 3 - 2 Multiplier
            ...
            lockupInfo[ _recipient ] = Lockup({
                gonsWarmup: IxVexaris( xVexaris ).gonsForBalance( _amount ),
                gonsAccount: IxVexaris( xVexaris ).gonsForBalance( _amount ),
                initialDeposit: _amount,
                multiplier: 200,
        ...
        xVexaris.safeTransfer( address(warmupContract), amountToTransfer );
        emit LogStake(_recipient, _amount);
        return true;
    }

    /**
        @notice retrieve xVexaris from warmup
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = xVexaris.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit xVexaris in warmup and retrieve Vexaris
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint memoBalance = xVexaris.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        Vexaris.safeTransfer( msg.sender, info.deposit);
    ...
    /**
        @notice redeem xVexaris for Vexaris
        @param _amount uint
        @param _trigger bool
     */
    function unstake( uint _amount, bool _trigger ) external {
        ...
        require(xVexaris.balanceOf(msg.sender) - _amount >= xVexaris.balanceForGons( lock.gonsAccount ), "Not enough xVexaris for lockup");
        xVexaris.safeTransferFrom( msg.sender, address(this), _amount );
        Vexaris.safeTransfer( msg.sender, _amount );
    ...
    function unstakeLocked(bool _trigger) external{
        ...
        if (lock.multiplier >0){
            xVexaris.safeTransferFrom( msg.sender, address(this), xVexaris.balanceForGons( lock.gonsAccount ));
            warmupContract.retrieve( address(this), xVexaris.balanceForGons( lock.gonsWarmup ));

            if (lock.lockTimestamp <= block.timestamp){
                Vexaris.safeTransfer( msg.sender, xVexaris.balanceForGons( lock.gonsWarmup + lock.gonsAccount ));
            }
            else{
                Vexaris.safeTransfer( msg.sender, lock.initialDeposit );
            }
    ...
    /**
        @notice returns the xVexaris index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return xVexaris.index();
    }
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            xVexaris.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = xVexaris.circulatingSupply();
    ...
    /**
        @notice returns contract Vexaris holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return Vexaris.balanceOf( address(this) ).add( totalBonus );
    }
