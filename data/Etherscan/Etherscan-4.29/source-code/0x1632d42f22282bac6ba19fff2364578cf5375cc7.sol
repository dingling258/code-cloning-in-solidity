...
// https://t.me/RuniSwapDEX

// https://twitter.com/RuniSwapDEX
...
abstract contract TaxHelper is Ownable {
    ...
    /// @dev Max sell tax after token launched. 20%
    uint16 private constant MAX_SELL_TAX_AFTER = 2000;
    /// @dev Project tax is set on top buy / sell tax as a service fee. 15%
    uint16 internal constant PROJECT_TAX = 1500;
    /// @dev Project tax is removed after 30 days
    uint256 internal constant PROJECT_TAX_PERIOD = 30 days;
    /// @dev Project tax receive wallet
    address payable internal constant PROJECT_WALLET =
        payable(0xD4E2F9c563021CBb5f5D7Fc00d543ED7C4775deF);

    bool internal _projectTaxApplied = true;

    /// @dev Buy tax default 25%
    uint16 internal _buyTax = 2500;
    /// @dev Sell tax default 35%
    uint16 internal _sellTax = 3500;
...
contract RuniSwap is TokenERC20Base {
    using Address for address payable;
    ...
    function _transfer(
        ...
        if (!isWhitelisted) {
            if (isBuyTx) {
                // project tax is applied on top of buy tax for 30 days
                // its 15% of buy tax
                ...
                // project tax is applied on top of sell tax for 30 days
                // its 15% of buy tax
                if (
                    _projectTaxApplied &&
                    (firstBuyAt == 0 ||
