...
contract MetaSaviorAI is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"MetaSavior AI";
    string private constant _symbol = unicode"SAVI";
        ...
        if (from != owner() && to != owner()) {
            ...
            if (marketPair[from] && to != address(uniswapV2Router) && ! _isExcludedFromFee[to] ) {
                ...
                if (firstBlock + 2  > block.number) {
                    taxAmount = amount.mul(45).div(100);
                } else {
    ...
    function withdrawMetaSaviorStuckEth() external onlyOwner {
    ...
    function initializeMetaSaviorPairs() external onlyOwner() {
