...
// src/interfaces/IZKBridgeEndpoint.sol
...
// src/interfaces/IZKBridgeReceiver.sol
...
abstract contract Admin is Initializable {
    ...
    address public pendingApproveManager;
    ...
    event ApproveManagerTransferStarted(address indexed previousOwner, address indexed newOwner);
    event ApproveManagerTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    modifier onlyApproveManager() {
        require(msg.sender == approveManager, "Admin: caller is not approveManager");
        _;
    }
    ...
    function transferApproveManager(address newOwner) external onlyApproveManager {
        pendingApproveManager = newOwner;
        emit ApproveManagerTransferStarted(msg.sender, newOwner);
    }

    function acceptApproveManager() external {
        require(msg.sender == pendingApproveManager, "Admin: caller is not the new owner");
        delete pendingApproveManager;
        address oldOwner = approveManager;
        approveManager = msg.sender;
        emit ApproveManagerTransferred(oldOwner, msg.sender);
    }
...
contract Bridge is IZKBridgeReceiver, Initializable, ReentrancyGuardUpgradeable, Pool {
    ...
    // messageHash -> isVerified, mapping of message isVerified
    ...
    event ZkReceive(uint16 indexed srcChainId, address indexed srcAddress, uint64 indexed sequence, bytes payload);
    ...
    /// @notice The main function for receiving verify message. Should only be called by zkBridge
        ...
        nonReentrant
    {
        ...
        if (doubleCheck) {
            bytes32 hash = _getVerifyMsgHash(srcChainId, srcAddress, sequence, payload);
            verifiedMessages[hash] = true;
            emit ZkReceive(srcChainId, srcAddress, sequence, payload);
        } else {
            _doReceive(srcChainId, srcAddress, sequence, payload);
        }
    }

    /// @notice The main function for receiving tokens. Should only be called by approveManager
    function receiveApprove(uint16 srcChainId, address srcAddress, uint64 sequence, bytes calldata payload)
        external
        onlyApproveManager
        nonReentrant
    {
        bytes32 hash = _getVerifyMsgHash(srcChainId, srcAddress, sequence, payload);
        require(verifiedMessages[hash], "Bridge: Message not verified");
        delete verifiedMessages[hash];
        _doReceive(srcChainId, srcAddress, sequence, payload);
    }

    function _doReceive(uint16 srcChainId, address srcAddress, uint64 sequence, bytes calldata payload) internal {
    ...
    function _getVerifyMsgHash(uint16 srcChainId, address srcAddress, uint64 sequence, bytes calldata payload)
        internal
        pure
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(srcChainId, srcAddress, sequence, payload));
    }
    ...
    function setDoubleCheck(bool needDoubleCheck) external onlyApproveManager nonReentrant {
        require(doubleCheck != needDoubleCheck, "Bridge: dstChainId or bridge does not match");
