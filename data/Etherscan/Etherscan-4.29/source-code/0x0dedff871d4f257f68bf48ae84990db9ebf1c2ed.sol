/**
 *Submitted for verification at testnet.bscscan.com on 2024-04-07
*/
...
contract ERC20Base is Context, ERC20, Ownable, Lockable {
    ...
    event VestingStarted(address to, uint256 amount, uint256 releaseTime);
    mapping(address => Vest[]) public vestings;
    mapping(address => Vest[]) public storedVestings;

    function getUserVesting(
        address to
    ) external view returns (UserVesting[] memory) {
        Vest[] memory vests = storedVestings[to];
        UserVesting[] memory userVestings = new UserVesting[](vests.length);
        for (uint256 i = 0; i < vests.length; i++) {
            userVestings[i].amount = vests[i].amount;
            userVestings[i].releaseTime = vests[i].releaseTime;
            if (userVestings[i].releaseTime <= block.timestamp) {
                userVestings[i].released = true;
            } else {
                userVestings[i].released = false;
            }
        }
        return userVestings;
    }
    ...
    function updateUserVestings(address to) internal {
        Vest[] storage userVestings = vestings[to];
        uint256 totalAllowed = 0;
        uint256 length = userVestings.length;
        uint256 i = 0;
        while (i < length) {
            if (userVestings[i].releaseTime <= block.timestamp) {
                totalAllowed += userVestings[i].amount;
                userVestings[i] = userVestings[length - 1];
                userVestings.pop();
                length--;
            } else {
                i++;
        ...
        if (totalAllowed > 0) {
            token.transfer(to, totalAllowed);
        }
    }

    function getAllowedVestedAmount(
        address to
    ) internal view returns (uint256 amount) {
        Vest[] memory userVestings = vestings[to];
        uint256 totalAllowed = 0;
        uint256 length = userVestings.length;
    ...
    function transferVested(
        ...
        uint256[] memory unlockDays
    ) external onlyOwner {
        require(
            amounts.length == unlockDays.length && amounts.length > 0,
            "Lengths doesn't match"
        );
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < amounts.length; i++) {
            amounts[i] = amounts[i] * (10 ** uint256(decimals()));
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            if (i > 0) {
                unlockDays[i] += unlockDays[i - 1];
            }
            userVestings.push(
                Vest({
                    to: to,
                    amount: amounts[i],
                    releaseTime: block.timestamp + (unlockDays[i] * 86400)
                })
            );
            storedVestings[to].push(
                Vest({
                    to: to,
                    amount: amounts[i],
                    releaseTime: block.timestamp + (unlockDays[i] * 86400)
    ...
    function balanceOf(
        ...
        return _balanceOf(account) + getAllowedVestedAmount(account);
    }
...
contract PYRAMIDCOIN is ERC20Base, ReentrancyGuard {
    uint256 public maxSupply = 5000000000000 * (10 ** uint256(decimals())); //maximum amount of tokens that can be minted

    constructor() payable ERC20Base("PYRAMID COIN", "PYDcoin") {
        mint(msg.sender, 1000000000000 * (10 ** uint256(decimals())));
    }
