...
contract Onionz is Ownable, ERC721A, ReentrancyGuard {
    ...
    constructor(address initialOwner) ERC721A("Onionz","ONNZ") Ownable(initialOwner) {
    ...
    function freeMint(uint256 amount) external payable callerIsUser {
        ...
        require(freeMintCount[msg.sender] + amount <= MAX_FREE_MINT, 'free mint reached per mint max amount');
        freeMintCount[msg.sender] += amount;
        _safeMint(msg.sender, amount);
    }

    function setFreeSale(bool _on) external onlyOwner {
        freeSaleOn = _on;
    }
