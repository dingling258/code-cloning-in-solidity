...
contract BullieverseTokenProtectedWrapped is ERC20, Adminable, IBEP20Extension, IWrappedToken {
    address public bridge;
    address public bridgeCandidate;

    address public antiBot;

    event BridgeChangeRequested(address indexed oldBridge, address indexed newBridge);
    event BridgeChangeConfirmed(address indexed oldBridge, address indexed newBridge);
    ...
    constructor(
        ...
        bridge = _bridge;
        emit BridgeChangeConfirmed(address(0), _bridge);
    ...
    modifier onlyBridge() {
        require(_msgSender() == bridge, "Auth failed");
        _;
    }

    modifier onlyBridgeCandidate {
        require(_msgSender() == bridgeCandidate, "Auth failed");
        _;
    }
    ...
    function changeBridge(address _bridgeCandidate) onlyBridge external {
        bridgeCandidate = _bridgeCandidate;
        emit BridgeChangeRequested(bridge, _bridgeCandidate);
    }

    function confirmNewBridge() onlyBridgeCandidate external {
        emit BridgeChangeConfirmed(bridge, bridgeCandidate);
        bridge = bridgeCandidate;
        bridgeCandidate = address(0);
    }

    function mintTo(address _recipient, uint256 _amount) external onlyBridge virtual override returns (bool) {
        _beforeTokenTransfer(address(0), _recipient, _amount);
        _mint(_recipient, _amount);
        return true;
    }

    function burnFrom(address _account, uint256 _amount) external onlyBridge virtual override returns (bool) {
