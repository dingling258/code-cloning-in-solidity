// File: contract/interfaces/IBRC20Factory.sol
...
// File: contract/BRC20Factory.sol
...
contract BRC20Factory  {
    ...
    bytes32 private constant DOMAIN_NAME = keccak256("BRC20Bridge");
    bytes32 public constant DOMAIN_TYPEHASH = keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)");
    ...
    function createBRC20(string memory name, string memory symbol, uint8 decimals) external onlyOwner returns (address newBRC20Address) {
        parameters = Parameters({name: name, symbol: symbol, decimals: decimals});
        newBRC20Address = address(new BRC20{salt: keccak256(abi.encode(name, symbol, decimals))}());
        delete parameters;
        emit BRC20Created(msg.sender, newBRC20Address);
    }
