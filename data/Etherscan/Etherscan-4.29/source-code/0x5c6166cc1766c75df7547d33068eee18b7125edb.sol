...
contract NGPURevenueStaking is ReentrancyGuard, DividendTracker {

    IERC20 public immutable ngpuToken;
    ...
    constructor(address _ngpuToken, uint256 _lockTimeInDays) {
        require(_ngpuToken != address(0), "cannot be 0 address");
        ngpuToken = IERC20(_ngpuToken);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        uint256 initialBalance = ngpuToken.balanceOf(address(this));
        ngpuToken.transferFrom(address(msg.sender), address(this), _amount);
        amountTransferred = ngpuToken.balanceOf(address(this)) - initialBalance;
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        ngpuToken.transfer(address(msg.sender), _amount);
    ...
    function withdrawAll() public nonReentrant {
        ...
        ngpuToken.transfer(address(msg.sender), userAmount);
    ...
    function compound(uint256 minOutput) external nonReentrant {
        ...
        if(amountEthForCompound > 0){
            ...
            uint256 amountTransferred = ngpuToken.balanceOf(address(this)) - initialBalance;
            setBalance(payable(msg.sender), userAmount + amountTransferred);
        } else {
            revert("No rewards");
        }
